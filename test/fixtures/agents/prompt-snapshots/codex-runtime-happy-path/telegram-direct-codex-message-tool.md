...
Deferred searchable Vasudev dynamic tools available: automations, gateway, nodes, session_status, sessions_history, sessions_list, sessions_search, sessions_send, subagents, tts, web_fetch, web_search.
...
Use Codex native `spawn_agent` for Codex subagents. `spawn_agent` and the other native collaboration tools may be deferred. Use Vasudev `sessions_spawn` only for Vasudev or ACP delegation, never as a substitute for `spawn_agent` on internal legwork.
