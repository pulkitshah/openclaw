...
- `src/entry.version-fast-path.ts:58` — the general fast path used for any other invocation shape (`isRootVersionInvocation`, checked before `dist/entry.js` even runs commander's own `.version()`/`isRootVersionInvocation` branch in `src/cli/program/help.ts:139-140`, which _does_ use `PRODUCT_NAME` correctly but is unreachable for `--version`/`-V`/`-v` because this fast path always wins first): ``output(commit ? `OpenClaw ${VERSION} (${commit})` : `OpenClaw ${VERSION}`);``
...
**Outside the enforced allowlist (re-running the apply script will _not_ touch these — a maintainer resolves them by hand):**
