...
Vasudev keeps its authoritative state in SQLite: one global control-plane
...
the state directory. Vasudev-owned SQLite databases, including agent databases
...
Prefer the supported Vasudev snapshot commands when you need one portable
recovery artifact.
...
pushes only content changes by construction. Vasudev stages only the
...
Use a repository dedicated to Vasudev backups. Existing `global/` and
`agents/<agentId>/` scopes must be empty or contain a valid schema-version-1
Vasudev backup manifest. Vasudev refuses to replace any other scope, and an
...
SQLite. It runs alongside the Gateway with no Vasudev changes: it watches
...
Litestream's one hard requirement is WAL mode, which Vasudev uses on local
filesystems; on network-backed storage such as NFS or SMB, Vasudev falls
...
required on the origin. Vasudev uses WAL on local filesystems but
...
Two caveats. First, deltas are page-based, and Vasudev's databases run
...
state directory or any configured live agent directory. Vasudev verifies
...
After restoring onto a different Vasudev version, preflight the database
