---
summary: "Run Vasudev Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
**Goal:** Vasudev Gateway running on an [exe.dev](https://exe.dev) VM, reachable at `https://<vm-name>.exe.xyz`.
...
Shelley, exe.dev's agent, can install Vasudev from a prompt:
...
<Steps>
  <Step title="Create the VM">
    ...
    <Tip>
    Keep this VM **stateful**. Vasudev stores `openclaw.json`, shared and per-agent SQLite auth stores, sessions, and channel/provider state under `~/.openclaw/`, plus the workspace under `~/.openclaw/workspace/`.
    </Tip>
  ...
  <Step title="Configure nginx to proxy to port 8000">
    ...
    Overwrite forwarding headers instead of preserving client-supplied chains. Vasudev trusts forwarded IP metadata only from explicitly configured proxies, and append-style `X-Forwarded-For` chains are treated as a hardening risk.
  ...
  <Step title="Trust nginx and allow the public browser origin">
    ...
    allowlist lets Vasudev use nginx's overwritten `X-Forwarded-For` value
  ...
  <Step title="Access Vasudev and approve devices">
