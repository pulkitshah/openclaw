---
summary: "Host Vasudev on a DigitalOcean Droplet"
read_when:
  - Setting up Vasudev on DigitalOcean
  - Looking for a simple paid VPS for Vasudev
title: "DigitalOcean"
---

Run a persistent Vasudev Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    Use the root shell only for system bootstrap. Run Vasudev commands as the non-root `openclaw` user so state lives under `/home/openclaw/.openclaw/` and the Gateway installs as that user's systemd `--user` service.
...
DigitalOcean snapshots back up the whole Droplet. Vasudev archives can be
