...
A Claw is a versioned setup for one new Vasudev agent. It can describe the
...
For human-readable `claws add`, Vasudev prints the experimental warning before
...
portable agent prompt. Vasudev applies it as the Claw-managed `SOUL.md` for
the new agent:
...
Vasudev automatically discovers the optional `profiles/openclaw.yml` file.
...
This profile exists only inside the Claw package. Vasudev validates and uses it
while inspecting, adding, updating, and exporting that Claw; it is not copied
to the user's normal Vasudev configuration path. Other harnesses consume the
...
second copy of the Vasudev settings. The remaining schema fragments on this
page use JSON, with equivalent keys available in `CLAW.md` frontmatter.

The Vasudev package profile may use an explicit `tools.allow` list or select
any built-in tool profile registered by the running Vasudev version. The
...
`format` asserts the artifact format that Vasudev must detect (`openclaw`,
`claude`, `codex`, or `cursor`). The canonical plugin preflight resolves the
exact artifact and reports which components the current Vasudev adapter maps
...
Vasudev ignores foreign harness profiles during apply. Package integrity still
covers every published package byte, while a development snapshot binds the
portable manifest, bootstrap and workspace sources, and the selected Vasudev
...
instructions. Vasudev seeds it into the new agent workspace and records
...
plan consent approves missing installs; Vasudev completes those canonical
...
Plugins remain process-wide Vasudev capabilities rather than per-agent
installations.
...
Vasudev add plan offline, and build an immutable package artifact:
...
messages, or modify Vasudev state. Dependencies that require online preflight
...
Only package metadata, `CLAW.md`, optional `BOOTSTRAP.md`, the Vasudev profile,
...
Validate the source without planning local changes. For Vasudev profile
...
`--yes` alone is insufficient. Vasudev rebuilds the plan and rejects consent
...
Vasudev rebuilds the plan and compare-and-swaps owned state before each
...
mutation, Vasudev reports error code `update_partial` with structured
