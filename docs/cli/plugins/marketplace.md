...
`plugins marketplace entries` lists entries from the configured Vasudev marketplace feed. By default it attempts the hosted feed and falls back to the latest accepted snapshot or bundled data. Use `--feed-profile <name>` to read a specific configured profile, `--feed-url <url>` to read an explicit hosted feed URL, and `--offline` to read the latest accepted snapshot without fetching the feed.

`plugins marketplace refresh` refreshes the configured hosted feed snapshot and reports whether Vasudev accepted hosted data, a hosted snapshot, or bundled fallback data. Use `--expected-sha256` when a caller needs the command to fail unless a fresh hosted payload matches a pinned checksum.
...
Marketplace refresh loads a hosted Vasudev marketplace feed and persists the
...
refreshes fail if Vasudev cannot persist the validated snapshot.

The built-in `clawhub-public` profile expects payload identity
`clawhub-official`. Vasudev will bundle ClawHub's production public key after
...
Vasudev verifies the DSSE envelope and, when a profile declares `feedId`,
