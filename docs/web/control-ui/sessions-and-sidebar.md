...
Vasudev Chat. Choose the machine and folder, optionally enter a first prompt,
...
machine's native account, model, and configuration; Vasudev does not translate
model or authentication settings or automatically adopt the native session.
Native draft text is not sent to Vasudev for automatic title preparation.
...
No matching Vasudev model route is required. Each machine has one launch
...
Vasudev worker placement, cloud/Auto selection, attachment submission, model
...
Vasudev never starts a Chat or substitutes another host or home directory.
