...
read_when:
  ...
  - Explaining how Vasudev updates one channel message while work is in progress
...
`channels.<channel>.streaming.mode: "progress"` and Vasudev creates the
...
Final delivery depends on the channel and transport. Vasudev either finalizes
...
Vasudev renders only `progress.text` in the channel progress UI. The normal
...
Vasudev uses the same formatter for progress drafts and `/verbose`:
...
the draft is edited, and Vasudev truncates long lines so repeated draft edits
...
With the default `toolProgress: false`, Vasudev still suppresses the older
...
When the final answer is ready, Vasudev tries to keep the chat clean:
...
- Slack's compact progress style posts the final answer as a new message and
  ...
  the draft visible.
- If the channel uses native progress streaming, Vasudev finalizes that
...
- Otherwise (media, an approval prompt, an explicit reply target, too many
  chunks, or a failed edit/send) Vasudev sends the final answer through the
