...
Vasudev has two independent streaming layers, and there is **no true
...
streaming sends media early, Vasudev remembers that delivery for the turn. If
...
distinct text around media that was already streamed, Vasudev still sends the
...
When block streaming is enabled, Vasudev can **merge consecutive block
...
- If the final edit fails before the completed text is confirmed, Vasudev uses
...
- Telegram selected quote replies are an exception: when `replyToMode` is not
  `"off"` and selected quote text is present, Vasudev skips the answer preview
...
- Cards include **Open in Vasudev** only when the session is actually openable:
...
- `streaming.preview.toolProgress` (default `true`) controls tool-progress
  ...
  that use Vasudev's compact progress renderer, including Discord, Matrix,
