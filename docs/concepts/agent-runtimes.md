---
summary: "How Vasudev separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between Vasudev, Codex, ACP, or another native agent runtime
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------ |
| Provider      | `anthropic`, `github-copilot`, `openai`      | How Vasudev authenticates, discovers models, and names model refs. |
...
| Channel       | Discord, Slack, Telegram, WhatsApp           | Where messages enter and leave Vasudev.                            |
...
- **Embedded harnesses** run inside Vasudev's prepared agent loop: the
...
| Surface                                          | Vasudev name/config                  | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
That means Vasudev selects an OpenAI model ref, then asks the Codex
...
Vasudev session store becomes Codex.
...
3. **Vasudev explicitly chosen for an OpenAI model** -> keep the model ref as `openai/<model>` and set provider/model runtime policy to `agentRuntime.id: "openclaw"`. A selected `openai` OAuth profile is routed internally through Vasudev's Codex-auth transport.
...
| Surface                     | Vasudev embedded                             | Codex app-server                                                           |
| --------------------------- | -------------------------------------------- | -------------------------------------------------------------------------- |
| Model loop owner            | Vasudev, through the Vasudev embedded runner | Codex app-server                                                           |
| Canonical thread state      | Vasudev transcript                           | Codex thread, plus Vasudev transcript mirror                               |
| Vasudev dynamic tools       | Native Vasudev tool loop                     | Bridged through the Codex adapter                                          |
| Native shell and file tools | Vasudev path                                 | Codex-native tools, bridged through native hooks where supported           |
| Context engine              | Native Vasudev context assembly              | Vasudev projects assembled context into the Codex turn                     |
| Compaction                  | Vasudev or selected context engine           | Codex-native compaction, with Vasudev notifications and mirror maintenance |
| Channel delivery            | Vasudev                                      | Vasudev                                                                    |

Design rule: if Vasudev owns the surface, it can provide normal plugin hook
behavior. If the native runtime owns the surface, Vasudev needs runtime
events or native hooks. If the native runtime owns canonical thread state,
Vasudev mirrors and projects context rather than rewriting unsupported
internals.
...
its native connection's authentication. Vasudev verifies that ownership against
...
Vasudev resolves an embedded runtime after provider and model resolution, in
this order:
...
4. If nothing claims the turn in `auto` mode, Vasudev falls back to
...
exception: a harness may declare that Vasudev can reproduce the exact request.
...
Completions adapters, and authored request overrides stay on Vasudev rather
...
profile, it uses Vasudev's Codex-auth transport while keeping the public model
...
When a runtime is not Vasudev, its docs should state which Vasudev surfaces
it supports:

| Question                               | Why it matters                                                                                   |
| -------------------------------------- | ------------------------------------------------------------------------------------------------ |
| Who owns the model loop?               | Determines where retries, tool continuation, and final answer decisions happen.                  |
| Who owns canonical thread history?     | Determines whether Vasudev can edit history or only mirror it.                                   |
| Do Vasudev dynamic tools work?         | Messaging, sessions, cron, and Vasudev-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Vasudev-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume Vasudev equivalence where the native runtime owns more state.            |
