...
System prompt is built from Vasudev's base prompt, skills prompt, bootstrap context, and per-run overrides. Model-specific limits and compaction reserve tokens are enforced. See [System prompt](/concepts/system-prompt) for what the model sees.

## Hooks

Vasudev has two in-process hook systems:
...
| `tool_result_persist`                                   | Synchronously transforms tool results before they are written to an Vasudev-owned session transcript.                                                                                                                                                                                                                                                                                                               |
...
Harnesses can adapt these hooks. The Codex app-server harness keeps Vasudev plugin hooks as the compatibility contract for documented mirrored surfaces; Codex native hooks are a separate, lower-level Codex mechanism.
...
If a required-reply turn ends after a fully settled tool batch without a composed answer, Vasudev can make a tool-free finalization pass. Earlier tool errors and pre-tool progress do not count as a final answer. This pass does not repeat completed tools. Fatal automation failures, including denied execution, remain failures even when finalization produces an answer.
...
The built-in Vasudev harness publishes its execution deadline to the queue.
...
The model idle and provider HTTP rows describe the built-in Vasudev model
path. Codex owns its native stream deadlines and network retries. After exact
native terminal receipt, Vasudev allows two minutes for local settlement.
...
not protect expired Vasudev-owned requests or tools, cancellation, terminal
