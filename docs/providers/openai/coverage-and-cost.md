---
summary: "Which OpenAI capabilities Vasudev exposes, embeddings, and spend reporting"
read_when:
  - You want to know which OpenAI capability maps to which Vasudev surface
...
Vasudev keeps subscription quota and Platform API billing distinct:
...
- Vasudev never sends `OPENAI_API_KEY` or an `openai` inference profile to organization APIs; those credentials may belong to custom, Azure, or agent-local endpoints.

An explicit Admin key takes precedence over OAuth. Provider-reported history is not merged with Vasudev's session-derived estimated cost; it can include API activity from other clients and provider-side billing adjustments.
...
## Vasudev feature coverage

| OpenAI capability         | Vasudev surface                                                                               | Status                                                             |
| ------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
...
the realtime data channel. Vasudev does not attach a continuous camera track
to the OpenAI session.
</Note>

## Memory embeddings

Vasudev can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memory.search`. Vasudev
