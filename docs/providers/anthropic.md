---
summary: "Use Anthropic Claude via API keys or Claude CLI in Vasudev"
read_when:
  - You want to use Anthropic models in Vasudev
  - You want to browse Claude CLI or Claude Desktop sessions across paired computers
title: "Anthropic"
---

Anthropic builds the **Claude** model family. Vasudev supports two auth routes:
...
Vasudev detects the available Anthropic credential and selects the matching usage surface:

- Vasudev-managed subscription/setup credentials show quota windows and optional extra-usage budget.
- Native Claude CLI logins stay under Claude's exclusive refresh control, so Vasudev does not poll their quota endpoint.
...
Admin API cost history comes from Anthropic's [Usage and Cost API](https://platform.claude.com/docs/en/manage-claude/usage-cost-api). It is actual provider billing, separate from Vasudev's session-derived estimated cost.

<Warning>
Claude Code owns its existing login and subscription; Vasudev does not persist
...
<Tabs>
  ...
  <Tab title="Claude CLI">
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      <Step title="Ensure Claude CLI is installed and logged in">
        Vasudev communicates directly with the installed Claude Code executable.
        ...
        Vasudev so the gateway launches the new binary:
      ...
      <Step title="Run onboarding">
        ...
        through Vasudev's direct CLI transport. Vasudev uses a non-secret route
        ...
        or remove its saved profile. Native-tool approvals remain under Vasudev
        control. Schema-valid native calls pass through Vasudev's canonical
    ...
    <Warning>
    Claude CLI reuse expects the Vasudev process to run on the same host as the
    ...
    a provider with Vasudev-managed OAuth such as
    ...
    **Anthropic setup-token** under **Connect with an API key or token**, or use:
    ...
    Vasudev release. Check `claude auth status`, `/status`, and
    ...
    Claude CLI. Vasudev also supports subscription-style options from
...
128,000-token output limit. Fable 5.1 always uses adaptive thinking. Vasudev defaults to
...
`fable-5` and `anthropic/claude-fable-5` selections still use Fable 5; Vasudev
...
Vasudev's Anthropic adapter converts a forced tool choice to `auto`
...
blocks. Claude Code manages this history for the CLI runtime. Vasudev's
...
invalidated replayed thinking server-side, and Vasudev logs a warning with the
...
On other eligible routes, a client-side prune is a one-time prefix edit. Vasudev
...
Anthropic drops those blocks unbilled, and Vasudev's embedded runtime omits
...
direct API-key routes Anthropic drops those blocks server-side and Vasudev
...
No additional Vasudev config is required for discovery. The Anthropic plugin
...
30 seconds while visible, so Claude sessions created outside Vasudev appear
...
`sessions.catalog.continue`. Vasudev re-resolves the local catalog record,
...
The new Vasudev session starts with the catalog title and color. Continuing an
already adopted session preserves any title or color changes made in Vasudev.
...
and its local `claude` executable resolves. Vasudev re-resolves the catalog
...
advertises `agent.cli.claude.run.v1`. Vasudev never modifies Claude Desktop
...
capabilities match the request shaping Vasudev would apply to it. A brand-new
model generation therefore stays hidden until Vasudev adds support for it,
...
`sonnet` tracks the current Sonnet. Upgrading Vasudev can therefore move a
...
higher native effort levels. Vasudev omits manual thinking budgets, custom
...
thinking. Vasudev defaults both versions to `medium` effort. Anthropic does not allow thinking to be
disabled for these models, so stored `off` and `minimal` settings map to `low`
effort instead. Vasudev also omits caller-selected sampling parameters for
both Fable versions.
...
stored `adaptive` selections resolve to Vasudev's `medium` default. Custom
...
adaptive-thinking and five-effort contract. Vasudev defaults to `high`, maps
...
Claude Opus 4.8 keeps thinking off by default in Vasudev. When you explicitly
enable adaptive thinking with `/think high|xhigh|max`, Vasudev sends
...
another Claude model. Vasudev opts into Anthropic's recommended per-category
...
1. For every direct API-key request to `anthropic/claude-opus-5`,
   `anthropic/claude-fable-5-1`, or `anthropic/claude-fable-5`, Vasudev sends the
...
2. Only a safety-classifier decline triggers the fallback. Rate limits,
   ...
   Vasudev's normal [model failover](/concepts/model-failover).
...
4. If the recommended model declines as well, the turn surfaces the refusal
   as an error. Vasudev does not retry a final refusal or advance to another
   configured model.

The fallback happens at the Anthropic API level, so the serving model does not
need to be in your configured Vasudev fallback chain.
...
- Anthropic bills the fallback attempt at the serving model's rates. Vasudev
...
- A mid-stream decline additionally bills the already-streamed primary-model partial
  ...
  usage but not folded into Vasudev's per-turn estimate.
...
Vasudev supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  ...
  <Accordion title="Bedrock Claude notes">
    ...
    - Supported Nova models offer opt-in explicit caching: set `cacheRetention` explicitly to `short` or `long` for system/message checkpoints with a five-minute TTL. Unset retention adds no checkpoints. Nova explicit caching has not been live-verified against AWS by Vasudev maintainers yet. Other non-Claude models remain at `cacheRetention: "none"`; see [Bedrock prompt caching](/reference/prompt-caching#amazon-bedrock) for model IDs, AWS limits, and the live acceptance proof gap.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    For Claude Opus 5 and Opus 4.8, Vasudev's shared `/fast` toggle uses
    ...
    <Note>
    - Native fast mode is a research preview for Claude Opus 5 and Opus 4.8. It can deliver up to 2.5x higher output-token throughput and is billed at `$10/$50` per million input/output tokens. Vasudev applies the same 2x multiplier to cache pricing in its cost estimate.
    ...
    - Claude Sonnet 5 supports neither native fast mode nor Priority Tier, so Vasudev omits both fields.
  ...
  <Accordion title="Server-side compaction">
    ...
    Vasudev adds the `compact-2026-01-12` beta header and sends an Anthropic
    `context_management` compaction edit. When compaction occurs, Vasudev
    ...
    When `anthropicCompactThreshold` is omitted, Vasudev uses
    ...
    <Warning>
    Anthropic server-side compaction is a beta feature and Vasudev never
    ...
    proxies, Bedrock, Vertex, and Foundry are excluded. Vasudev does not send
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. Vasudev
    ...
    When an image or PDF is attached to a conversation, Vasudev automatically
  ...
  <Accordion title="1M context window">
    ...
    thinking: Opus 4.8,
    Opus 4.7, Opus 4.6, and Sonnet 4.6. Vasudev sizes these models
    ...
    these models and Vasudev no longer sends the retired
    ...
    extended context. Vasudev uses CLI-owned metadata and configured limits;
    ...
    <Warning>
    Requires long-context access on your Anthropic credential. OAuth/subscription token auth keeps its required Anthropic beta headers, but Vasudev strips the retired 1M beta header if it remains in older config.
    </Warning>
...
<AccordionGroup>
  <Accordion title="Claude CLI OAuth session expired or could not be refreshed">
    ...
    Claude Code owns its login and refresh lifecycle; do not copy an OAuth token into Vasudev.
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    ...
    Native Claude CLI logins remain owned by Claude Code, not the shared Vasudev auth store. For that route, use the [Claude CLI setup](/providers/anthropic#getting-started); do not copy native OAuth tokens into Vasudev.
