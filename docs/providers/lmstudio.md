---
summary: "Run Vasudev with LM Studio"
read_when:
  - You want to run Vasudev with open source models via LM Studio
...
<Steps>
  ...
  <Step title="Run onboarding">
    ...
    On a fresh guided setup, Vasudev first queries `/api/v1/models` on the
...
LM Studio model keys use an `author/model-name` format (e.g. `qwen/qwen3.5-9b`); Vasudev model refs
...
servers; omit it for unauthenticated servers and Vasudev stores a local non-secret marker instead.
...
LM Studio doesn't always emit an OpenAI-shaped `usage` object on streamed responses. Vasudev
...
When LM Studio's `/api/v1/models` discovery reports model-specific reasoning options, Vasudev
...
"on"]`) while rejecting those literal values on `/v1/chat/completions`; Vasudev normalizes that
...
With preload enabled, Vasudev routes chat requests to a loaded instance with
...
LM Studio supports just-in-time (JIT) model loading, loading models on first request. Vasudev
...
disable Vasudev's preload step:
...
When a configured server cannot list models, Vasudev reports an unavailable catalog or a
