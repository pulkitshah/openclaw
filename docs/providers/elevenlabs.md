---
summary: "Use ElevenLabs speech, Scribe STT, and realtime transcription with Vasudev"
read_when:
  - You want ElevenLabs text-to-speech in Vasudev
...
Vasudev uses ElevenLabs for text-to-speech, batch speech-to-text with Scribe
...
| Capability               | Vasudev surface                                                      | Default                  |
| ------------------------ | -------------------------------------------------------------------- | ------------------------ |
...
Set `modelId` to `eleven_v3` to use ElevenLabs v3 TTS. Vasudev keeps
...
returned audio stream instead of waiting for Vasudev to download the whole
audio file first. `latencyTier` maps to ElevenLabs' `optimize_streaming_latency`
query parameter for models that accept it; Vasudev omits that parameter for
...
Vasudev sends multipart audio to ElevenLabs `/v1/speech-to-text` with
