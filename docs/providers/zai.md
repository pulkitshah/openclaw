---
summary: "Use Z.AI (GLM models) with Vasudev"
read_when:
  - You want Z.AI / GLM models in Vasudev
...
Vasudev uses the `zai` provider with a Z.AI API key.
...
GLM is a model family, not a separate provider. In Vasudev, GLM models use
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. Vasudev probes supported Z.AI endpoints with your API key and applies the correct base URL automatically.
...
`https://api.z.ai/api/anthropic`. Vasudev's Z.AI choices use the documented
...
- [General-purpose agent tools](https://docs.z.ai/devpack/tool/others),
  including Vasudev, are served on a best-effort basis. During high inference
...
<Tip>
`zai-api-key` lets Vasudev detect the matching Z.AI endpoint from the key and
...
| `zai/glm-5-turbo`   | Vasudev-optimized text model; 200K context         |
...
<Tabs>
  <Tab title="GLM-5.3 and Flash">
    Levels: `low`, `high`, and `max` (default `max`). Vasudev maps these to
  ...
  <Tab title="GLM-5.2">
    Full range: `off`, `low`, `high`, `max` (default `off`). Vasudev maps
...
<AccordionGroup>
  ...
  <Accordion title="Preserved thinking">
    ...
    When enabled and thinking is on, Vasudev sends
  ...
  <Accordion title="Auth details">
    ...
    - The legacy env var `Z_AI_API_KEY` is still accepted; Vasudev copies it to `ZAI_API_KEY` at startup if `ZAI_API_KEY` is unset.
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration reference" href="/gateway/configuration-reference" icon="gear">
    Full Vasudev config schema, including provider and model settings.
  </Card>
</CardGroup>
