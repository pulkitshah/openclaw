...
```json5
{
  gateway: {
    ...
    controlUi: {
      ...
      // communityInvite: false, // opt in to the upstream Discord invitation in the sidebar
...
- `tailscale.mode`: `serve` (tailnet only, loopback bind) or `funnel` (public, requires auth).
  Vasudev holds the route as a foreground claim, so startup fails unless the
...
- `tailscale.preserveFunnel`: deprecated migration guard. When `true` and
  `tailscale.mode = "serve"`, Vasudev checks `tailscale funnel status` before
  ...
  provenance. Vasudev leaves the external route unchanged and warns. The
...
- `controlUi.communityInvite`: show the upstream project's Discord community invitation in the sidebar. Default: `false`. Set `true` on the Gateway serving the UI to offer it to every browser using that deployment, including browsers connected to a different remote Gateway. The setting hot-reloads; existing pages pick it up after browser refresh or reconnect. Disabling and re-enabling preserves browser-local dismissals.
...
- `terminal.shell`: optional shell executable. When unset, Vasudev uses `$SHELL` on Unix and `%ComSpec%` on Windows. Changes hot-apply to newly opened terminals; existing terminals keep running their original shell.
