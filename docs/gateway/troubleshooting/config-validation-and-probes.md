...
- Vasudev keeps the latest 32 `.clobbered.*` files for each config path and rotates older ones.
...
<AccordionGroup>
  <Accordion title="What happened">
    ...
    - When many repairs happen for one config path, Vasudev rotates older `.clobbered.*` files so the newest repaired payload is still available.
...
- `multiple reachable gateway identities detected` → distinct gateways answered, or Vasudev could not prove reachable targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is treated as one gateway with multiple transports, even when transport ports differ.
