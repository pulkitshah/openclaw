...
| `label`                      | `"INBOX"`                    | Gmail label to watch. Vasudev excludes `SPAM`, `TRASH`, `DRAFT`, and `SENT` when launching the watcher.                                                       |
...
| `pushToken`                  | required                     | Authenticates incoming pushes to the watcher. Separate from `hooks.token`, which authenticates forwarding to Vasudev. Setup generates one if absent.          |
