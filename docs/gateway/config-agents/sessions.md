...
- **`identityLinks`**: map canonical ids to provider-prefixed peers for cross-channel session sharing. The Duties plugin's Team roster writes this map for its own members automatically, always paired with `session.dmScope: "per-peer"` on that member's binding — `per-peer` is required there because `resolveLinkedDirectPeerId` is only consulted when `dmScope` is not `"main"`.
