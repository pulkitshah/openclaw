---
summary: "Use OpenShell as a managed sandbox backend for Vasudev agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend: Vasudev delegates sandbox lifecycle to
...
Vasudev Gateway, which continues to run the agent and host-side plugins.
...
- `openshell` CLI installed and on the Vasudev Gateway process's `PATH` (or a
...
- A supported compute runtime on the OpenShell gateway host when using local
  sandboxes
- Vasudev Gateway running on the host
...
Before configuring Vasudev, verify the OpenShell CLI as the same operating
system user that runs the Vasudev Gateway:
...
The Vasudev Gateway service must see the same OpenShell CLI, gateway
...
Validate the configuration and restart the Vasudev Gateway:
...
On the next agent turn Vasudev creates an OpenShell sandbox and routes tool
...
- Before `exec`, Vasudev syncs the local workspace into the sandbox.
- After `exec`, Vasudev syncs the remote workspace back to local.
- Within one Vasudev Gateway process, commands and file-tool operations sharing
...
Best for development workflows: local edits outside Vasudev show up on the
...
- On first use after sandbox creation, Vasudev seeds the remote workspace
...
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate
  directly on the remote workspace. Vasudev does **not** sync remote changes
  back to local.
...
<Warning>
Editing files on the host outside Vasudev after the initial seed is invisible to the remote sandbox. Run `openclaw sandbox recreate` to re-seed.
</Warning>
...
| `policy`                  | `string`                 | unset         | Path to a sandbox policy YAML file on the Vasudev Gateway host                         |
...
rejected. Choose distinct, non-overlapping directories because Vasudev manages
...
It cannot select different OpenShell workspaces per Vasudev agent or session.
Changing it does not migrate existing sandboxes. Delete Vasudev's OpenShell
...
Vasudev keeps a registered sandbox's shipped legacy runtime name after an
...
upgrade. Vasudev preserves the registered runtime identity, refuses to create
...
If OpenShell cannot delete a sandbox, Vasudev reports the failure and keeps the
...
the Vasudev Gateway host instead of being copied into an untrusted sandbox.
...
Vasudev does not apply Docker image, root-filesystem, network, user, or package
settings to this backend.

Custom images used with the Vasudev filesystem bridge must provide:
...
First separate Vasudev Gateway health, plugin activation, and OpenShell
...
- **Plugin missing or backend unavailable:** Install
  ...
  validate the config, and restart the Vasudev Gateway. Run
...
- **Workspace missing or the wrong sandbox list:** Verify the selected
  ...
  enabling them in Vasudev. Remember that the Gateway service may not inherit
...
- **Mirror synchronization reports recovery paths:** Vasudev kept a host shadow
  ...
  Vasudev preserves remaining workspace entries instead of overwriting them
...
- **Recreate or prune cannot delete a sandbox:** Restore access to the original
  ...
  the scoped recreation. Vasudev retains the registry entry until deletion
  succeeds.

## How it works

1. Vasudev runs `sandbox get` for the sandbox name (with the selected
...
2. Vasudev runs `sandbox ssh-config` for the sandbox name to fetch SSH
   connection details.
