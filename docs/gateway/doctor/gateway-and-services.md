...
<AccordionGroup>
  ...
  <Accordion title="15. Supervisor config audit + repair">
    ...
    - Within a live Linux service inspection, Vasudev can retain the authenticated user manager's private connection if its session bus stops. Typed command and runtime reads continue only for that original manager and unit. A missing initial manager identity or a replaced manager remains unavailable; Vasudev does not start the bus or select another manager. This read-only recovery does not change service start/stop authority.
    ...
    - Doctor service repairs refuse to rewrite, stop, or restart a gateway service from an older Vasudev binary when the config was last written by a newer version. See [Gateway troubleshooting](/gateway/troubleshooting#split-brain-installs-and-newer-config-guard).
