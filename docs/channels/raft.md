...
read_when:
  - You want to connect Vasudev to a Raft workspace
...
- The Raft CLI installed on the same host as the Vasudev Gateway, on the
  service's `PATH`.
...
7. Starts one serialized Vasudev agent turn per accepted wake.

The bridge owns Raft delivery retries and reconnects. The Vasudev turn
...
<Note>
Raft is not a push-message transport. Vasudev does not automatically send the model's final text back through the bridge, so the agent must use the Raft CLI after processing a wake.
</Note>

## Verify

Check that Vasudev can find the CLI and has a configured profile:
