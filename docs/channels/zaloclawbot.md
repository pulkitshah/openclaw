...
Vasudev connects to Zalo ClawBot through the catalog-listed external `@zalo-platforms/openclaw-zaloclawbot` plugin. Login uses a Zalo Mini App QR code; the plugin id in config is `openclaw-zaloclawbot`.

<Note>
  This plugin's runtime lives entirely in the external `@zalo-platforms/openclaw-zaloclawbot` package. Behavior details on this page beyond install and config are as reported by the plugin's maintainers and are not verified against Vasudev core source.
</Note>

## Compatibility

| Plugin Version | Vasudev Version | npm dist-tag | Status        |
| -------------- | --------------- | ------------ | ------------- |
...
- Node.js >= 22
- [Vasudev](/install) installed (`openclaw` CLI available)
...
Use the exact pinned version so Vasudev verifies the package against the catalog integrity hash during install.
...
The plugin manages bot credentials under the Vasudev state directory. Treat that directory as sensitive and cover it under the same access-control and backup policy as the rest of Vasudev state.
...
- **Gateway fails to load:** confirm your Vasudev host version is `2026.4.10` or higher. Older versions do not support the external npm-plugin installation ledger this ID requires.
