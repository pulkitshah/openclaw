---
summary: "Messaging platforms Vasudev can connect to"
read_when:
  - You want to choose a chat channel for Vasudev
  - You need a quick overview of supported messaging platforms
title: "Chat channels"
---

Vasudev can talk to you on any chat app you already use. Each channel connects via the Gateway.
...
"External plugin" channels are maintained outside the Vasudev repo.
...
- [A2A](/channels/a2a) - Connect external agents to Vasudev through the A2A 1.0 JSON-RPC protocol (bundled plugin).
- [Buzz](/channels/buzz) - Connect Vasudev agents to Buzz rooms (official plugin).
...
- [iMessage](/channels/imessage) - Native iMessage support via imsg (JSON-RPC over stdio), with private API actions for replies, tapbacks, effects, polls, attachments, and group management. Preferred for new Vasudev iMessage setups when host requirements fit (official plugin).
...
- [Mattermost](/channels/mattermost) - Mattermost bot setup and Vasudev config (official plugin).
...
- [Reef](/channels/reef) - Reef channel setup: guarded, end-to-end-encrypted messaging between Vasudev agents of different people (bundled plugin).
...
- [Synology Chat](/channels/synology-chat) - Synology Chat webhook setup and Vasudev config (official plugin).
...
- Channels can run simultaneously; configure multiple and Vasudev will route per chat.
