...
read_when:
  ...
  - Enabling the plugin-tools or Vasudev-tools MCP bridge
...
| `openclaw`   | Vasudev ACP bridge (native `openclaw acp`)                                                             |
...
When Vasudev uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal Vasudev `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by Vasudev before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, Vasudev/acpx cannot force a model selection.
...
ACPX isolates bare session names by Vasudev owner. If a session reports
...
manually, and there is no separate postinstall step for Vasudev itself. If an
adapter download or spawn fails, `/acp doctor` reports the failure.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose Vasudev plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
Vasudev plugin tools such as memory recall/store, enable the dedicated bridge:
...
- Injects a built-in MCP server named `openclaw-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled Vasudev
  plugins.
...
### Vasudev tools MCP bridge

By default, ACPX sessions also do **not** expose built-in Vasudev tools through
...
- Injects a built-in MCP server named `openclaw-tools` into ACPX session
  bootstrap.
- Exposes selected built-in Vasudev tools. The initial server exposes `cron`.
...
Runtime turns use Vasudev agent/run timeouts, including `/acp timeout`.
...
These ACPX harness permissions are separate from Vasudev exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.

For the broader comparison between Vasudev `tools.exec.mode`, Codex Guardian
...
<Warning>
Vasudev defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `PermissionPromptUnavailableError: Permission prompt unavailable in non-interactive mode`.
