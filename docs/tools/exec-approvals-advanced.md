...
- If Vasudev cannot identify exactly one concrete local file for an interpreter/runtime command
...
session, Vasudev also resumes that session with an internal followup so the agent observes that
...
After an approved async exec finishes, Vasudev sends a followup `agent` turn to the same session.
...
channel, Vasudev keeps the local deterministic `/approve` prompt visible. If the native runtime is
active and attempts delivery but no target receives the card, Vasudev sends a same-chat fallback
...
- Telegram defaults to approver DMs (`target: "dm"`). Switch to `channel` or `both` to also show
  approval prompts in the originating Telegram chat/topic. For Telegram forum topics, Vasudev
...
- Matrix native DM/channel routing and reaction shortcuts handle both exec and plugin approvals;
  ...
  include `com.openclaw.approval` custom event content on the first prompt event so Vasudev-aware
...
owner routing for approval prompts and final results. Vasudev first tries a private route on the
