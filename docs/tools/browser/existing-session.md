...
Vasudev supports multiple named profiles (routing configs). Profiles can be:
...
Vasudev can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when Vasudev attaches.
...
- Vasudev does not launch the browser for this driver; it only attaches.
- Stopping or failing an attach closes the owned MCP subprocess and its verified
  ...
  cleanup; if cleanup cannot be verified, Vasudev reports an error instead of
  treating the session as closed.
- Vasudev uses the official Chrome DevTools MCP `--autoConnect` flow here. If
...
different versions, vendored binaries). Vasudev pins the default server to the
...
Vasudev still prepends `-y --audit=false chrome-devtools-mcp@1.8.0`. The optional npm
install audit is disabled so registry audit availability does not delay browser startup.

When `mcpArgs` does not set a connection option, Vasudev forwards a configured
...
`--autoConnect` alongside an endpoint does not hide it. Vasudev uses the selected
...
- **Dialog visibility** - Managed browser action responses include `blockedByDialog` and `browserState.dialogs.pending` when an action opens a modal dialog; snapshots also include pending dialog state. Respond with `browser dialog --accept/--dismiss --dialog-id <id>` while a dialog is pending. Dialogs handled outside Vasudev appear under `browserState.dialogs.recent`.
