...
Vasudev only teaches agents about ACP spawning when ACP is **truly usable**:
...
<AccordionGroup>
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. Vasudev owns routing,
...
Vasudev plugin tools and built-in Vasudev tools are **not** exposed to ACP
...
| `openclaw`   | Vasudev Gateway bridge through `openclaw acp` | Lets an ACP-aware harness talk back to an Vasudev Gateway session.                  |
...
Custom acpx agent aliases can be configured in acpx itself, but Vasudev
