---
summary: "Index of the Vasudev ACP agents documentation, one page per reader job"
read_when:
...
Vasudev run external coding harnesses (Claude Code, Cursor, Copilot, Droid,
Vasudev ACP, OpenCode, Gemini CLI, and other supported acpx harnesses)
...
existing Vasudev channel conversations, use
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ Vasudev | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                 |
| Expose an Vasudev Gateway session _as_ an ACP server for an editor or client                   | [`openclaw acp`](/cli/acp)            | Bridge mode: an IDE/client speaks ACP to Vasudev over stdio/WebSocket                                                                                                       |
| Reuse a local AI CLI as a text-only fallback model                                             | [CLI Backends](/gateway/cli-backends) | Not ACP: no Vasudev tools, no ACP controls, no harness runtime                                                                                                              |
...
is enabled. Use **sub-agents** when you want Vasudev-native delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | Vasudev native sub-agent runtime   |
...
1. Vasudev ACP session control plane.
...
the plugin-tools and Vasudev-tools MCP bridges, and ACP permission modes,
