...
summary: "How Vasudev structures the built-in agent runtime: code layout, boundaries, resource manifests, and runtime selection."
---

Vasudev owns the built-in agent runtime. Runtime code lives under `src/agents/`, model/provider transport lives under `src/llm/`, and `openclaw/plugin-sdk/*` barrels expose the plugin-facing contracts.
...
| `src/agents/runtime/`               | Vasudev facade that wires `@openclaw/agent-core` to the plugin SDK LLM runtime and re-exports it plus local proxy utilities.                                                                                              |
| `src/agents/agent-tools*.ts`        | Vasudev-owned tool definitions, parameter schemas, tool policy, before/after tool-call adapters, and host/sandbox edit tools.                                                                                             |
...
Core calls the built-in runtime through Vasudev modules and SDK barrels. No external agent framework packages remain. Plugins use documented `openclaw/plugin-sdk/*` entrypoints and do not import `src/**` internals.
...
Resource packages declare Vasudev resources in `package.json` metadata. Entries are file paths or globs relative to the package root:
...
- `auto` selects a registered plugin harness that supports the effective provider route, otherwise the built-in Vasudev runtime. A provider or model prefix alone never selects a harness.
...
- [Vasudev agent runtime workflow](/openclaw-agent-runtime)
