---
summary: "Vasudev on Omarchy: bar plugin, desktop app handoff, and support"
read_when:
  - Installing the Vasudev bar plugin on Omarchy
  - Using agents, sessions, and quick prompts from the Omarchy bar
  - Troubleshooting duplicate Vasudev icons or desktop connection handoff
title: "Omarchy"
---

# Vasudev on Omarchy

The Vasudev Omarchy plugin puts agents, recent sessions, and a quick prompt in
...
This page covers the plugin in the Vasudev source tree. Install it separately;
...
- Either a connected Vasudev Linux desktop app with Omarchy integration, or
...
Use your existing Vasudev installation and authentication. In standalone
...
Click the Vasudev mascot. Confirm that the panel shows the expected agents
...
and your existing Vasudev updater for the CLI and Gateway. After updating
...
| Two Vasudev icons                          | Update both the plugin and desktop app to versions with the integration. They must run in the same desktop user session.                            |
| CLI not found                              | Check `OPENCLAW_DESKTOP_CLI`, `~/.openclaw/bin/openclaw`, or the graphical session's `PATH`. Install through your normal Vasudev installation path. |
...
For setup help, use [Vasudev support](/help). For a reproducible plugin or
handoff defect, file an [Vasudev bug report](https://github.com/openclaw/openclaw/issues/new?template=bug_report.yml)
