...
- **Every prompt seems to overflow on a small local model?** Confirm the provider reports the correct model context window. Vasudev can cap the effective reserve only when that window is known.
