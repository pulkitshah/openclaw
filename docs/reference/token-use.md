---
summary: "How Vasudev builds prompt context and reports token usage + costs"
read_when:
...
Vasudev tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Vasudev assembles its own system prompt on every run. It includes:
...
Vasudev derives the live tool-result cap from the effective model context
...
publish a `1050000` token total window, but Vasudev defaults their active
...
For images, Vasudev downscales transcript/tool image payloads before
...
- `/usage cost` -> local cost summary from Vasudev session logs.
...
Vasudev falls back to matching OAuth/API-key credentials from auth
...
Vasudev keeps provider usage accounting separate from the current context
...
Provider prompt caching only applies within the cache TTL window. Vasudev
...
Vasudev sizes GA-capable Claude 4.x models such as Opus 4.8, Opus 4.7, Opus
...
Older configs can keep `context1m: true`, but Vasudev no longer sends
...
(`sk-ant-oat-*`), Vasudev preserves the OAuth-required Anthropic beta
