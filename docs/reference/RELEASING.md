...
Vasudev exposes four user-facing update channels:
...
- Plugin version sync updates the publishable `@openclaw/ai` runtime package, official plugin package versions, and existing `openclaw.compat.pluginApi` floors to the Vasudev release version by default. Treat that field as the plugin SDK/runtime API floor, not just a copy of the package version: for plugin-only releases that intentionally remain compatible with older Vasudev hosts, keep the floor at the oldest supported host API and document that choice in the plugin release proof.
...
- `source=npm`: `openclaw@beta`, `openclaw@latest`, or an exact Vasudev release version
...
**Vasudev Release Button**, selecting the same protected tooling tag. This is
...
positive numeric child IDs. Start a new **Vasudev Release Prepare** run on the
...
| Failure                                                                   | Recovery                                                                                                                                                                                    |
| ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Outer readiness seal/download fails after both child preparations succeed | On **Vasudev Release Prepare**, rerun **Verify and seal prepared publication**. It reuses the child run IDs, resolves their current attempts once, and does not dispatch another build.     |
...
| Publisher download fails before writes                                    | Start an explicit new **Vasudev Release Button** run with the same `prepared_artifact`. No version bump or repack is needed.                                                                |
...
Vasudev NPM Release child**. If npm contains the core version but that child
...
To attach Windows assets later or retry a failed promotion, use the exact Vasudev tag, exact published Windows source tag, and approved installer digests:
...
- `npm_dist_tag`: npm target tag for the Vasudev package, one of `alpha`, `beta`, `latest`, or `extended-stable`
...
6. Run `OpenClaw Release Publish` from the protected `release-publish/<sha12>-<epoch>` tooling tag with the same `tag`, the same `npm_dist_tag`, the optional Windows input pair, the saved `preflight_run_id`, `full_release_validation_run_id`, and `full_release_validation_run_attempt`. It starts plugin npm and ClawHub in parallel, then promotes the prepared Vasudev npm package once plugin npm succeeds. GitHub finalization waits for npm and Docker evidence; apps attach independently afterward.
