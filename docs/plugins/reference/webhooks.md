---
summary: "Authenticated inbound webhooks that bind external automation to Vasudev TaskFlows."
read_when:
  - You are installing, configuring, or auditing the webhooks plugin
title: "Webhooks plugin reference"
---

<!-- Generated file. Do not edit by hand.
Run `pnpm plugins:inventory:gen` to rebuild it. Hand-written text survives only
between the openclaw-plugin-reference:manual-start and
openclaw-plugin-reference:manual-end comment markers. -->

Authenticated inbound webhooks that bind external automation to Vasudev TaskFlows.

## Distribution

- Package: `@openclaw/webhooks`
- Install route: included in Vasudev

## Surface

This plugin declares no channels, providers, commands, or contracts.

## Related docs

- [webhooks](/plugins/webhooks)
