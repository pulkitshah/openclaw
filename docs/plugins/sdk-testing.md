---
summary: "Testing utilities and patterns for Vasudev plugins"
...
Reference for test utilities, patterns, and lint enforcement for Vasudev
plugins.
...
These subpaths are repo-local source entrypoints for Vasudev's own bundled
...
| `createOpenClawTestState` / `withOpenClawTestState` / `OpenClawTestState` | Create and clean up isolated Vasudev state, config, workspace, environment, and auth-profile fixtures. Import from `plugin-sdk/test-state` |
...
Core-only suites that depend on bundled Vasudev inventory stay under
...
exercise Vasudev's loader acceptance gates. Add at least one loader-backed
...
Vasudev uses Vitest 5 with informational V8 coverage reporting. For plugin tests:
