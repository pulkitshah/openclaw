...
The timeout budgets around a Codex turn, and how Vasudev settles a completed turn. Part of the [Codex harness reference](/plugins/codex-harness-reference); [Where each section moved](/plugins/codex-harness-reference#where-each-section-moved) lists every section.

## Timeouts

Vasudev-owned dynamic tool calls are bounded independently from
...
On timeout, Vasudev aborts the tool signal where supported and returns a failed
...
completion. For ordinary turns, Vasudev waits for `turn/completed` with the
...
failure, but Vasudev does not upgrade a timeout or lost client into success.
...
On receipt of the exact native terminal event, Vasudev starts an absolute
...
preserves that answer as a degraded success. Vasudev retires unfinished
...
existing five-second drain grace; if the writer remains unavailable, Vasudev
...
Vasudev requests, tools, cancellation, terminal cleanup, or ownerless state.
...
Vasudev then reports the failure rather than automatically rerunning the work.
