---
summary: "What Vasudev core prepares and owns before a harness runs an attempt, and the contracts a harness can declare to take some of it back"
read_when:
...
What Vasudev prepares before it calls `runAttempt`, and the narrow contracts a harness declares to own tool policy, auth bootstrap, a bound native session, or its own request transport. Part of the [Agent harness plugins](/plugins/sdk-agent-harness) reference.

## What core still owns

For ordinary concrete-model turns, Vasudev prepares these inputs before
calling `runAttempt`:
...
- the Vasudev transcript/session file
...
explicit Vasudev tool-policy layer across native and built-in tools, Vasudev
...
canonical Vasudev tool names in `conversationToolPolicyNativeTools`. Core checks
...
`conversationToolPolicySafeDenyTools` using canonical Vasudev tool names. Core
...
Vasudev then visibly rejects explicitly restricted turns before invoking the
...
Core still forwards a compatible, explicitly selected or ordered Vasudev auth
...
Verified Vasudev continuations also pass `params.expectedRuntimeArtifact`.
...
Harnesses may use the plan for decisions that need to match Vasudev behavior,
...
all of them before dispatch. Implicit selection uses Vasudev if no plugin can
own the full set; an explicit or persisted plugin selection fails closed unless
the plugin declares the lossless Vasudev fallback.
...
local date and time zone as the built-in Vasudev runtime. It uses
