...
read_when:
  - You want to know which Vasudev tools reach a Codex turn
  - You are debugging turn liveness or parallel chats
  - You need the ownership split between Vasudev and Codex
title: "Codex runtime behavior"
sidebarTitle: "Runtime behavior"
---

What the Codex harness owns during a turn, and what stays with Vasudev. Part of the [Codex harness](/plugins/codex-harness) guide; [Where each section moved](/plugins/codex-harness#where-each-section-moved) lists every section.

## Dynamic tools and web search

Codex dynamic tools default to `searchable` loading. Vasudev normally does
...
so Vasudev does not project a second goal store into Codex turns. Most
remaining Vasudev integration tools, such as messaging, media, cron,
...
Vasudev instead publishes the distinct `gateway_exec` and `gateway_process`
names so native shell and the Vasudev-managed environment path cannot be
confused.

Tools marked `catalogMode: "direct-only"`, including the Vasudev `computer`
...
Vasudev's managed `web_search` dynamic tool are mutually exclusive so
managed search cannot bypass native domain restrictions. Vasudev uses the
managed tool when hosted search is unavailable, explicitly disabled, or
replaced by a selected managed provider. Vasudev keeps Codex's standalone
...
while explicit Vasudev or ACP delegation remains directly callable through
`sessions_spawn`. In Codex Code Mode, generic Vasudev
...
`view_image` and Vasudev suppresses its duplicate loader. The native Codex
schema accepts one local filesystem `path`. For text-only models, or when the
native tool surface is disabled, Vasudev supplies `view_image` with its
...
Codex owns provider-stream liveness and native turn completion. Vasudev waits
...
Vasudev still bounds its own requests, dynamic tools, cancellation, and local
...
Vasudev does not infer classifier activity from the assistant's wording.
...
cyber-policy refusal, Vasudev retries it once on the configured Daybreak model
...
- The retry does not mirror the prompt into the transcript a second time. The
  ...
  turn still exists upstream in the native Codex thread, which Vasudev does not
  rewrite.
...
full reconnect ladder. Vasudev therefore treats the retry itself as the only
...
continuation, or interruption, Vasudev keeps the original refusal even when
...
reads to finish. Vasudev coordinates its own lifecycle operations for each
...
existing configuration. Vasudev retains the configured native thread, including
for `/codex resume` of that chat's already-bound thread. Native provider policy refusals
end the current attempt without a native retry. Vasudev's configured cyber
...
loaded failed thread still requires native unloading. Vasudev preserves the
...
can operate outside Vasudev's thread queue. Avoid concurrently reconfiguring the
...
- Vasudev dynamic tools are supported. Codex asks Vasudev to execute
  those tools, so Vasudev remains in the execution path.
- Codex-native shell, patch, MCP, and native app tools are owned by Codex.
  Vasudev can observe or block selected native events through the
  supported relay, but it does not rewrite native tool arguments.
- `gateway_exec` and `gateway_process` are Vasudev-owned dynamic tools. They
...
- Media generation, media understanding, TTS, approvals, and messaging-tool
  output continue through the matching Vasudev provider/model settings.
- `tool_result_persist` applies to Vasudev-owned transcript tool results,
