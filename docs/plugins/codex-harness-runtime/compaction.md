---
summary: "Native Codex compaction, the Vasudev transcript mirror, and continuity projection"
read_when:
...
Who owns compaction on a Codex turn, and what the Vasudev transcript mirror keeps. Part of the [Codex harness runtime](/plugins/codex-harness-runtime) guide; [Where each section moved](/plugins/codex-harness-runtime#where-each-section-moved) lists every section.

## Compaction and transcript mirror

When the selected model uses the Codex harness, Codex app-server owns native
token-pressure and manual thread compaction. Vasudev separately owns its
transcript mirror. When `agents.defaults.compaction.maxActiveTranscriptBytes`
is set to a positive value, Vasudev checks that mirror before ordinary and
heartbeat turns. When the byte guard trips, Vasudev requires semantic
...
After host mirror compaction commits, Vasudev may request
`thread/compact/start` to synchronize an eligible native thread. This request
is secondary: Vasudev does not send it for host-isolated operations or
...
Vasudev keeps the request and shared-client lease open until Codex emits the
...
timeout, Vasudev requests a native turn interrupt. The lease and per-thread
...
period, Vasudev retires the connection before releasing the fence. Remote
...
synchronization described above, Vasudev starts native compaction only for
...
If context-engine compaction rotates the Vasudev session generation, the next
...
When Vasudev projects an existing session's continuity into a fresh Codex
...
When a context engine requests Codex thread-bootstrap projection, Vasudev
...
typed `thinking` content rather than ordinary final-answer text, so Vasudev's
existing reasoning visibility and history controls apply. Vasudev records
