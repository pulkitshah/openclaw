...
What Vasudev does about registered Codex app-server children when the Gateway stops without cleaning up. Part of the [Codex harness runtime](/plugins/codex-harness-runtime) guide; [Where each section moved](/plugins/codex-harness-runtime#where-each-section-moved) lists every section.

## Recovery after a hard Gateway stop

On POSIX systems, Vasudev checks for registered orphaned Codex app-server
...
Vasudev records the parent and child process identities in the current state
...
Cleanup only targets a registered child whose original Vasudev parent is no
...
Vasudev instance, processes registered under another state directory, and externally
...
unregistered children left by an older Vasudev version or scan command names
