...
default `messages.queue.mode: "steer"`, Vasudev batches steer-mode chat
...
When Codex confirms consumption, Vasudev saves completed visible assistant
...
that case, Vasudev waits for the active run to finish before starting the
...
harness, Vasudev also calls Codex app-server `feedback/upload` for relevant
...
Vasudev session ids, Codex thread ids, and local `codex resume <thread-id>`
commands for the threads that were sent.

If you deny or ignore the approval, Vasudev does not print those Codex ids
