...
`systemPrompt` replaces the complete Vasudev generic developer policy. An explicit
...
those transports. Use Vasudev-managed local stdio; for lease contention, stop
...
| Layer                                 | Owner                   | Purpose                                                             |
| ------------------------------------- | ----------------------- | ------------------------------------------------------------------- |
| Vasudev plugin hooks                  | Vasudev                 | Product/plugin compatibility across Vasudev and Codex harnesses.    |
| Codex app-server extension middleware | Vasudev bundled plugins | Per-turn adapter behavior around Vasudev dynamic tools.             |
| Codex native hooks                    | Codex                   | Low-level Codex lifecycle and native tool policy from Codex config. |

Vasudev does not use project or global Codex `hooks.json` files to route
plugin behavior. For the native tool and permission bridge, Vasudev injects
per-thread Codex config for `PreToolUse`, `PostToolUse`, `PermissionRequest`,
and `Stop`.

Before starting or resuming a thread, Vasudev prepares the native relay's
...
so Codex's app-server reviewer and Vasudev's approval bridge handle real
...
controls; they are not exposed as Vasudev plugin hooks in the v1 contract.

For Vasudev dynamic tools, Vasudev executes the tool after Codex asks for
...
canonical tool record; Vasudev can mirror selected events but cannot rewrite
...
request for the same tool use, Vasudev opens the plugin approval prompt and
maps the decision back to Codex. Codex `PermissionRequest` events are a
separate approval path and can still route through Vasudev approvals when
...
notifications and Vasudev adapter state, not native Codex hook commands.
...
debugging. They do not invoke Vasudev plugin hooks.
