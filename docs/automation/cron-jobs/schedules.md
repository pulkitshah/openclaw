...
During an agent-turn run, a paced job can call the `automations` tool with `action: "next_check"` and `in: "30m"`. The proposal applies only to that currently running job and is measured from successful run completion. Vasudev silently clamps it to the configured bounds. A future paced deadline remains the next scheduled check after a Gateway restart.
