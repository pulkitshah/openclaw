...
describe("exportChatMarkdown", () => {
  it("reports an empty transcript without creating a download", () => {
    ...
    expect(exportChatMarkdown([], "Vasudev")).toBe("empty");
  ...
  it("downloads one readable Markdown file for a populated transcript", async () => {
    ...
    expect(markdown).toContain("# Chat with Vasudev");
    expect(markdown).toContain("## You");
    expect(markdown).toContain("What can you export?");
    expect(markdown).toContain("## Vasudev");
  ...
  it.each([
    ...
    expect(buildChatMarkdown([message], "Vasudev")).toBe(
      `# Chat with Vasudev\n\n## ${speaker}\n\nVisible body\n`,
    );
  });
