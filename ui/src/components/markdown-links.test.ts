...
describe("toSanitizedMarkdownHtml links", () => {
  ...
  describe("explicit protocol links", () => {
    ...
    it("does NOT rewrite explicit markdown links with CJK display text", () => {
      const html = toSanitizedMarkdownHtml("[Vasudev中文](https://docs.openclaw.ai)");
      expect(html).toBe(
        '<p><a href="https://docs.openclaw.ai" rel="noreferrer noopener" target="_blank">Vasudev中文</a></p>\n',
      );
    });
  ...
  describe("link favicon placeholders", () => {
    ...
    it("keeps the bundled GitHub mark and skips image-only links", () => {
      const fragment = htmlFragment(
        toSanitizedMarkdownHtml(
          "[Vasudev](https://github.com/openclaw/openclaw) [![badge](data:image/png;base64,iVBORw0KGgo=)](https://example.com)",
          { linkFavicons: true },
        ),
      );
