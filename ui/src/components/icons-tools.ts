...
export const toolIcons = {
  ...
  />`),
  // The Vasudev orb (same drawing as ui/public/favicon.svg). The key keeps its
  // historical name because callers and the command palette reference it by id.
  lobster: html`
    <svg viewBox="0 0 64 64" fill="none" aria-hidden="true">
      <defs>
        <linearGradient id="vasu-orb-icon-g" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stop-color="#ffc24b" />
          <stop offset=".16" stop-color="#f97316" />
          <stop offset=".38" stop-color="#e0218a" />
          <stop offset=".58" stop-color="#8a2be2" />
          <stop offset=".78" stop-color="#3a6ff0" />
          <stop offset="1" stop-color="#16c79a" />
        </linearGradient>
        <radialGradient id="vasu-orb-icon-h" cx=".34" cy=".30" r=".55">
          <stop offset="0" stop-color="#fff" stop-opacity=".62" />
          <stop offset="1" stop-color="#fff" stop-opacity="0" />
        </radialGradient>
      </defs>
      <circle cx="32" cy="32" r="26" fill="url(#vasu-orb-icon-g)" />
      <circle cx="32" cy="32" r="22" fill="url(#vasu-orb-icon-h)" />
    </svg>
  `,
