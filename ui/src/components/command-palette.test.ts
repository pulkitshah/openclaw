...
describe("CommandPalette lifecycle", () => {
  ...
  it.each([
    ...
    "shows Ask Vasudev only when availability is $available",
    async ({ available, expectedCount }) => {
      const { gateway } = createGateway(true);
      const { palette } = await mountPalette(
        createContext(
  ...
  it("opens Ask Vasudev from its palette action", async () => {
    const { gateway } = createGateway(true);
    const { palette } = await mountPalette(
      createContext(
        gateway,
    ...
    try {
      findPaletteOption(palette, "Ask Vasudev", true)?.click();
    } finally {
