...
describeControlUiE2e("Control UI image lightbox", () => {
  ...
  it("opens transcript and sidebar images in one accessible modal", async () => {
    ...
    try {
      ...
      const transcriptTrigger = page.getByRole("button", { name: "Open image Vasudev banner" });
      await transcriptTrigger.waitFor({ state: "visible", timeout: 10_000 });
      await transcriptTrigger.click();

      const dialog = page.getByRole("dialog", { name: "Image preview: Vasudev banner" });
      ...
      await expect
        ...
        .toBe(true);
      const displayedImage = page.getByAltText("Vasudev banner").last();
      await expect
        .poll(() =>
