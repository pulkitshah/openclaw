...
export const zh_TW = {
  ...
  wizard: {
    ...
    gateway: {
      ...
      plaintextPasswordHint: "直接把密碼存入 Vasudev 設定",
    ...
    memoryImport: {
      ...
      confirm: "將這些 memory 匯入你的 Vasudev 工作區？",
    ...
    remote: {
      ...
      plaintextTokenHint: "將密鑰直接儲存在 Vasudev 設定中",
    ...
    guided: {
      laneQuestion: "你想如何開始？",
      laneSecurityLine:
        "Vasudev 執行的 AI 代理程式擁有這台機器的實際存取權限。安全指南：https://docs.openclaw.ai/gateway/security",
      ...
      custodianIntro: "你好 — 我是 Vasudev，負責維護這套系統。我們開始設定吧。",
      ...
      setupDone: "一切就緒。",
      appliedTitle: "推理已就緒",
      complete: "Vasudev 已準備就緒。",
      completeWithoutAi: "Vasudev 設定已儲存。連接 AI 後再開啟聊天。",
      detected: "AI 偵測完成。",
      ...
      intro: "連接你的 AI",
      invalidConfigDetails: "Vasudev 設定 {path} 無效：\n{issues}",
      invalidConfigRepair:
    ...
    security: {
      ...
      attribution: "Vasudev 是由 Vasudev 基金會（非營利組織）開發的開源助手，會與你一同學習成長。",
      baselineDmSessions:
      ...
      confirm: "我理解 Vasudev 預設面向個人使用；共享/多使用者使用需要加固。繼續？",
      hardeningRequired: "如果你不熟悉存取控制和安全加固，請在獲得協助前不要執行 Vasudev。",
      learnMore: "了解更多請參閱",
      notMultitenant: "預設情況下，Vasudev 並非旨在安全地隔離多個使用者。",
      personalAgent: "預設情況下，Vasudev 是供一位操作者使用的個人 agent。",
    ...
    channels: {
      ...
      commandOwnerHelp:
        "聊天存取權限允許使用者與你的代理交談。命令擁有者還可以更新 Vasudev、重新啟動 Gateway、變更設定並核准命令。請僅選擇你自己信任的管理員帳號。伺服器和群組無需 DM 配對也可使用此功能；它不會授予聊天存取權限或變更頻道存取規則。",
    ...
    clickclack: {
      ...
      helpCreateToken:
        "在 ClickClack 中：工作區設定 → Integrations → Vasudev → 建立 bot → 複製 token",
    ...
    nextcloudTalk: {
      ...
      helpInstallCommand:
        '2) 執行：./occ talk:bot:install "Vasudev" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction',
    ...
    whatsapp: {
      ...
      ownerNumberNote: "需要傳送者/擁有者號碼，以便 Vasudev 將你加入允許清單。",
      ...
      separatePhoneLabel: "Vasudev 專用獨立手機",
    ...
    synologyChat: {
      ...
      incomingWebhookHelpReplies: "這是 Vasudev 用來向 Chat 傳送回覆的 URL。",
      ...
      publicWebhookUrlScope: "僅公開此 webhook 路由；Vasudev 使用它提供短期附件下載。",
    ...
    signal: {
      ...
      multipleEntries: "多個項目請用逗號分隔。",
      nextLinkDevice: '綁定裝置：signal-cli link -n "Vasudev"',
    ...
    channelsPrimer: {
      ...
      blurbs: {
        ...
        synologyChat: "將 Synology NAS Chat 連接到 Vasudev，並啟用完整 agent 能力。",
    ...
    finalize: {
      ...
      dashboardCopyPaste: "在本機瀏覽器中複製/貼上這個 URL 來控制 Vasudev。",
      dashboardOpened: "已在瀏覽器中開啟。保留該分頁以控制 Vasudev。",
      ...
      optionalApps: "可選應用",
      outroDashboardLink: "Onboarding 完成。使用上面的 dashboard 連結控制 Vasudev。",
      outroDashboardOpened: "Onboarding 完成。Dashboard 已開啟；保留該分頁以控制 Vasudev。",
      ...
      containerSystemdUnavailable:
        "此容器內沒有 systemd 使用者服務。Vasudev 只會略過背景服務安裝；請以前景方式執行 Gateway，或使用你的容器 supervisor。",
      securityReminder:
