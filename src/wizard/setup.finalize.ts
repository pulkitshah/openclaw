    ...
    await params.prompter.note(
      [
        ...
        t("wizard.finalize.startGatewayNow", {
          command: formatCliCommand("vasudev gateway run"),
        }),
      ].join("\n"),
      "Gateway",
    );
...
function buildGatewayRecoveryProjection(params: {
  ...
  if (gateway.status === "ready") {
    ...
    const detail = t("wizard.finalize.managedGatewayUnreachable", {
      service,
      statusCommand: formatCliCommand("vasudev gateway status --deep"),
      recoveryCommand: formatCliCommand("vasudev gateway restart"),
    });
  ...
  if (gateway.status === "failed") {
    ...
    const detail = t("wizard.finalize.managedGatewaySetupFailed", {
      ...
      error: gateway.error,
      statusCommand: formatCliCommand("vasudev gateway status --deep"),
      recoveryCommand: formatCliCommand("vasudev gateway install --force"),
    });
    return {
      detail,
  ...
  return {
    detail: [
      ...
      t("wizard.finalize.rerunInstallDaemon", {
        command: formatCliCommand("vasudev onboard --install-daemon"),
      }),
      t("wizard.finalize.skipHealthNextTime", {
        command: formatCliCommand("vasudev onboard --skip-health"),
      }),
    ].join("\n"),
    summary,
  };
        ...
        await prompter.note(
          [
            ...
            t("wizard.finalize.noModelAuthNext", {
              command: formatCliCommand("vasudev configure --section model"),
            }),
          ].join("\n"),
      ...
      if (gatewayProbe.ok) {
        const tokenNotes = [
          ...
          t("wizard.finalize.gatewayTokenView", {
            command: formatCliCommand("vasudev gateway auth-token --show"),
          }),
          t("wizard.finalize.gatewayTokenGenerate", {
            command: formatCliCommand("vasudev doctor --generate-gateway-token"),
          }),
          t("wizard.finalize.dashboardOpenAnytime", {
            command: formatCliCommand("vasudev dashboard --no-open"),
          }),
        ].filter(Boolean);
      ...
      if (!entry) {
        await prompter.note(
          [
            ...
            `  ${formatCliCommand("vasudev configure --section web")}`,
            "",
        ...
        await prompter.note(
          [
            ...
            t("wizard.finalize.webSearchReenable", {
              command: formatCliCommand("vasudev configure --section web"),
            }),
            "",
            t("wizard.finalize.webDocs"),
          ].join("\n"),
