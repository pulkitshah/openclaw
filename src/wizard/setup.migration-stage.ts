      ...
      try {
        ...
        try {
          ...
          if (current && hashSetupMigrationConfig(current) === journal.configHashTarget) {
            ...
            await writePromotionJournal(journalPath, journal);
            throw new Error(
              `Migration config commit is indeterminate. Review ${journalPath} and run vasudev doctor before retrying.`,
              { cause: error },
            );
          }
        }
        ...
        await writePromotionJournal(journalPath, journal);
        throw new Error(
          `Migration promotion could not be rolled back. Review ${journalPath} and run vasudev doctor before retrying.`,
          { cause: error },
        );
      }
    },
