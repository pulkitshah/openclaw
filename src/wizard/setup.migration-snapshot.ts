...
/** Serializes onboarding writes that share one Vasudev state target. */
export async function withSetupMigrationTargetLock<T>(
  ...
  if (activeStateDir) {
    if (activeStateDir !== resolvedStateDir) {
      throw new Error("nested onboarding target lock cannot switch the Vasudev state directory");
    }
    return await fn();
  }
