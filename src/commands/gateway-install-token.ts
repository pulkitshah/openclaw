...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("vasudev config set gateway.auth.mode token")} or ${formatCliCommand("vasudev config set gateway.auth.mode password")}.`,
  ].join(" ");
}
...
export async function resolveGatewayInstallToken(
  ...
  if (!token && !tokenRefConfigured && options.requireExisting) {
    unavailableReason = "Deferred service load requires an existing Gateway token.";
