...
/** Relinks npm-owned plugin packages to the current Vasudev host package. */
export async function maybeRepairPluginOpenClawHostLinks(
  ...
  if (!params.prompter.shouldRepair) {
    ...
    if (audit.peerLinkIssues.length > 0) {
      note(
        [
          "Managed npm Vasudev host peer links need repair:",
          ...audit.peerLinkIssues.map((issue) => `- ${issue.packageName}: ${issue.reason}`),
          `Repair with ${formatCliCommand("vasudev doctor --fix")} to relink managed npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
    ...
    if (audit.registeredPeerLinkIssues.length > 0) {
      note(
        [
          "Registered npm plugin Vasudev host links need repair:",
          ...audit.registeredPeerLinkIssues.map(
            (issue) => `- ${issue.packageName}: ${issue.reason}`,
          ),
          `Repair with ${formatCliCommand("vasudev doctor --fix")} to relink registered npm plugin packages.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (warnings.length > 0) {
    note(
      ["Could not repair all managed npm Vasudev host peer links:", ...warnings].join("\n"),
      "Plugin registry",
    );
  }
