...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("vasudev automations list")} and ${formatCliCommand("vasudev automations show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
function collectCronDeliveryTargetAdvisory(params: {
  ...
  return [
    ...
    `Reactivate the channel plugin or update the job's \`delivery.channel\` after reviewing with ${formatCliCommand("vasudev automations list")} and ${formatCliCommand("vasudev automations show <job-id>")}.`,
  ].join("\n");
}
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.openclaw/bin/ensure-whatsapp.sh` is not maintained by current Vasudev and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("vasudev channels status --probe")}, ${formatCliCommand("vasudev doctor")}, and ${formatCliCommand("vasudev gateway status")} for current health checks.`,
