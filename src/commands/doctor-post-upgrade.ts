...
export async function runPostUpgradeProbes(params: {
  ...
  if (!installs) {
    findings.push({
      ...
      message:
        "Installed plugin index is missing, unreadable, or malformed. Run `vasudev plugins registry --refresh` to rebuild it before post-upgrade validation.",
    });
  ...
  for (const entry of drift.drifts) {
    ...
    findings.push({
      ...
      message: `Plugin ${entry.pluginId} is ${entry.installedVersion}, but Vasudev is ${VERSION}. ${updateCommand ? `Run \`${updateCommand}\`, then restart the Gateway.` : "No confirmed repair target is available; check registry availability and rerun this command."}`,
    });
  }
    ...
    if (pkgRelPath) {
      ...
      try {
        ...
        findings.push({
          ...
          message: `Plugin ${record.pluginId}: could not read package.json (${pkgRelPath}): ${reason}. Reinstall the plugin or run \`vasudev plugins registry --refresh\`.`,
      ...
      if (resolvedEntries.status === "invalid") {
        findings.push({
          level: "error",
          code: "plugin.entry_unresolved",
          message: `Plugin ${record.pluginId}: ${resolvedEntries.error}. Reinstall the plugin or run \`vasudev plugins registry --refresh\`.`,
          plugin: record.pluginId,
          entry: pkgRelPath,
        });
    ...
    if (record.manifestPath && record.manifestHash) {
      ...
      if (currentHash && currentHash !== record.manifestHash) {
        findings.push({
          ...
          message: `Plugin ${record.pluginId} manifest hash drifted from installs.json snapshot. Run \`vasudev plugins registry --refresh\` to re-sync.`,
          plugin: record.pluginId,
        });
      }
    }
