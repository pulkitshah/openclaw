...
export function buildGatewayRuntimeHints(
  ...
  if (runtime.cachedLabel && platform === "darwin") {
    ...
    hints.push(`Then reinstall: ${formatCliCommand("vasudev gateway install", env)}`);
  }
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("vasudev gateway install", env)}`);
    if (fileLog) {
      hints.push(`File logs: ${fileLog}`);
    }
    return hints;
  ...
  if (missingGuiSession || runtime.status === "stopped") {
    if (!missingGuiSession && platform === "linux" && isSystemdStartLimitHit(runtime)) {
      ...
      hints.push(
        ...
        `Recover with: ${formatCliCommand("vasudev gateway restart", env)}, then inspect logs if it keeps crashing.`,
      );
  ...
  if (platform === "linux" && isSystemdCgroupHygieneRisk(runtime.systemd)) {
    ...
    if (summary) {
      hints.push(
        ...
        `After reviewing service settings, run: ${formatCliCommand("vasudev gateway restart", env)}`,
      );
    }
  }
  return hints;
