...
describe("noteSecurityWarnings gateway exposure", () => {
  ...
  it("warns when exposed without auth", async () => {
    ...
    expect(findings).toEqual([
      expect.objectContaining({
        ...
        remediation: expect.stringContaining("vasudev doctor --fix"),
      }),
    ]);
    ...
    expect(message).toBe(
      [
        ...
        "  Or set token directly: vasudev config set gateway.auth.mode token",
        "- Run: vasudev security audit --deep",
      ].join("\n"),
    );
  });
  ...
  it("clarifies approvals.exec forwarding-only behavior", async () => {
    ...
    expect(message).toContain("vasudev approvals get --gateway");
  });
  ...
  it("explains how to renew inactive generated exec approvals", async () => {
    await withExecApprovalsFile(
      ...
      async () => {
        ...
        expect(finding?.remediation).toContain("vasudev doctor --fix");
  ...
  it("warns when model provider API keys are stored as plaintext in config", async () => {
    ...
    expect(message).toContain("vasudev secrets audit --check");
  });
  ...
  it("warns when tools.exec is broader than host exec defaults", async () => {
    ...
    expect(message).not.toContain("Vasudev default");
  });
  ...
  it("degrades safely when channel account resolution fails in read-only security checks", async () => {
    ...
    expect(message).toContain("Run: vasudev security audit --deep");
  });
