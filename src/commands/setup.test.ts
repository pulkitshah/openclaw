  ...
  it("explains that plain setup only initializes local files", async () => {
    await withTempHome(async (home) => {
      ...
      expect(runtime.log.mock.calls.map((call) => String(call[0])).slice(-5)).toStrictEqual([
        ...
        "Next guided path: vasudev onboard.",
        "Next targeted changes: vasudev configure for models, channels, Gateway, plugins, skills, and health checks.",
        "Add a chat channel later: vasudev channels add.",
      ]);
    });
  });
  ...
  it.each([false, true])(
    ...
    async (json) => {
      await withTempHome(async (home) => {
        ...
        expect(runtime.error).toHaveBeenCalledWith(expect.stringContaining("vasudev doctor"));
        if (json) {
          expect(runtime.log).toHaveBeenCalledOnce();
          expect(JSON.parse(String(runtime.log.mock.calls[0]?.[0]))).toEqual({
            ok: false,
            error: {
              type: "cli_error",
              message: "Vasudev config is invalid: ~/.openclaw/openclaw.json",
            },
