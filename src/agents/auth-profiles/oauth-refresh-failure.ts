...
export function buildOAuthRefreshFailureLoginCommand(
  ...
  if (sanitizedProvider === "claude-cli") {
    ...
    // first, then re-register that auth method with Vasudev.
    const claudeLoginCommand = formatCliCommand("claude auth login");
    const openclawLoginCommand = formatCliCommand(
      sanitizedProfileId
        ? `vasudev models auth login --provider anthropic --method cli --profile-id ${quoteShellArg(sanitizedProfileId)}`
        : "vasudev models auth login --provider anthropic --method cli",
    );
  ...
  return sanitizedProvider
    ? formatCliCommand(
        sanitizedProfileId
          ? `vasudev models auth login --provider ${sanitizedProvider} --profile-id ${quoteShellArg(sanitizedProfileId)}`
          : `vasudev models auth login --provider ${sanitizedProvider}`,
      )
    : formatCliCommand("vasudev models auth login");
}
...
export function buildAuthProfileUnusableHint(params: {
  ...
  if (
    ...
    if (params.provider === "google-gemini-cli") {
      ...
      const command = formatCliCommand("vasudev models auth login --provider google");
      return `Gemini CLI OAuth cannot be repaired by Vasudev. Connect Google with an AI Studio API key using ${formatOAuthRefreshFailureLoginCommandMarkdown(command)}, then select that Google profile for the Gemini CLI runtime.`;
    }
