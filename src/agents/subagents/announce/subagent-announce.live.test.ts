    ...
    async () => {
      ...
      await client.request<AgentPayload>(
        "agent",
        {
          ...
          timeout: 300,
          message: [
            "Run this exact Vasudev subagent scenario. Use tool calls, not prose.",
    ...
    async ({ skip }) => {
      ...
      const initialRequest = client.request<AgentPayload>(
        "agent",
        {
          ...
          timeout: 240,
          message: [
            "Run this exact Vasudev busy-parent subagent scenario. Use tool calls, not prose.",
            `Use nonce ${nonce}.`,
  ...
  it(
    ...
    async ({ skip }) => {
      ...
      await fs.writeFile(
        ...
        "Vasudev live stress test workspace. Keep responses concise.\n",
        "utf8",
      );
      await state.writeConfig(
        liveSubagentConfig(modelConfig.modelKey, state.workspaceDir, port, token, {
      ...
      const initialRequest = client.request<AgentPayload>(
        "agent",
        {
          ...
          timeout: 420,
          message: [
            "Run this exact Vasudev Gemini subagent stress scenario. Use tool calls, not prose.",
            `Use nonce ${nonce}.`,
