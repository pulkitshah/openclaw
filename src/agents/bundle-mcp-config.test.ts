/** Tests merging bundled MCP defaults with Vasudev user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets Vasudev mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps Vasudev transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled Vasudev MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled Vasudev MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
