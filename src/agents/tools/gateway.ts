  ...
  try {
    ...
    if (isNodeApprovalReplay) {
      throw new Error(
        [
          ...
          "Fix the Vasudev state directory permissions and retry the approval.",
        ].join(" "),
        { cause: error },
      );
    }
    throw new Error(
      [
        ...
        "Fix the Vasudev state directory permissions or use the local approval-runtime gateway.",
      ].join(" "),
      { cause: error },
    );
  }
