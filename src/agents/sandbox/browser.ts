      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `vasudev sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `vasudev sandbox recreate --browser --agent ${agentId}`;
          }
          return "vasudev sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
