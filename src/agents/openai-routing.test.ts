// Verifies OpenAI model selections route between Vasudev and Codex runtimes.
...
describe("OpenAI runtime routing policy", () => {
  ...
  ])("keeps %s values on the Vasudev runtime", (_label, params) => {
    const config = {
      agents: {
        defaults: {
          models: {
  ...
  it("fails closed to Vasudev when the provider artifact is unavailable", () => {
  ...
  it("honors the deprecated whole-agent Vasudev runtime opt-out", () => {
    const config = {
      agents: {
  ...
  it("keeps per-model Codex policy above the whole-agent Vasudev opt-out", () => {
    const config = {
      agents: {
        defaults: {
  ...
  it("keeps per-model auto policy above the whole-agent Vasudev opt-out", () => {
    const config = {
      agents: {
        defaults: {
          agentRuntime: { id: "openclaw" },
  ...
  it("keeps explicit Vasudev plus Codex auth profile under the unified OpenAI provider", () => {
  ...
  it("keeps explicit OpenAI Vasudev API-key auth order ahead of Codex backups", () => {
    const config = {
      auth: {
        order: {
  ...
  it("does not route custom OpenAI-compatible Vasudev configs through Codex auth order", () => {
    const config = {
      models: {
        providers: {
          openai: {
