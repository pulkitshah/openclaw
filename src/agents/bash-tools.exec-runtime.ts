...
function emitExecProcessCompleted(params: {
  ...
  // Payload stays untrusted, but the ambient trace context is the Vasudev run
  ...
  switch (params.failureKind) {
    ...
    case "overall-timeout": {
      ...
      return params.processContinuationAvailable
        ? `${retryGuidance}\n\nIf it should keep running, start it with exec background=true or yieldMs so Vasudev can register a pollable process session. Do not rely on shell backgrounding with a trailing &.`
        : retryGuidance;
    }
