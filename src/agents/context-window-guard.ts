...
export function formatContextWindowWarningMessage(params: {
  ...
  if (params.guard.source === "modelsConfig") {
    return (
      `${base}; Vasudev is using the configured model context limit for this model, ` +
...
export function formatContextWindowBlockMessage(params: {
  ...
  return (
    ...
    `Vasudev local/self-hosted runs work best at ${params.guard.warnBelowTokens}+ tokens.`
  );
}
