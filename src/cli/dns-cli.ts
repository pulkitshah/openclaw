  ...
  dns
    ...
    .action(async (opts) => {
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by vasudev dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (!cfg.discovery?.wideArea?.domain?.trim()) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: set discovery.wideArea.domain in the active Vasudev config ($OPENCLAW_CONFIG_PATH, default ~/.openclaw/openclaw.json) on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
