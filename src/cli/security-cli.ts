...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["vasudev security audit", "Run a local security audit."],
          [
            "vasudev security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["vasudev security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "vasudev security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "vasudev security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["vasudev security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["vasudev security audit --json", "Output machine-readable JSON."],
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Vasudev security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("vasudev security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("vasudev security audit --fix")}`));
        if (!fixResult) {
