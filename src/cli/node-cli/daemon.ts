...
function renderNodeServiceStartHints(): string[] {
  return buildPlatformServiceStartHints({
    installHint: formatCliCommand("vasudev node install"),
    startCommand: formatCliCommand("vasudev node start"),
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("vasudev node install --force")}`);
    }
    return;
  }
