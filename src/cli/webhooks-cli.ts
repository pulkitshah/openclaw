...
export function registerWebhooksCli(program: Command) {
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Vasudev hooks")
    ...
    .option("--hook-url <url>", "Vasudev hook URL")
    .option("--hook-token <token>", "Vasudev hook token")
...
function parseGmailSetupOptions(raw: Record<string, unknown>): GmailSetupOptions {
  ...
  if (!account) {
    throw new Error(
      `--account is required. Example: ${formatCliCommand("vasudev webhooks gmail setup --account default")}.`,
    );
  }
