...
export function formatUnknownChannelMessage(params: {
  ...
  const listCommand = params.listCommand ?? "vasudev channels list --all";
...
export function formatUnsupportedChannelActionMessage(params: {
  ...
  const inspectCommand =
    params.inspectCommand ?? `vasudev channels capabilities --channel ${params.channel}`;
...
export function formatStrictJsonParseFailure(params: { value: string; cause: unknown }): string {
  ...
  return [
    ...
    `Use valid JSON. For structured changes, use a JSON5 config patch object file with ${formatInlineCliCommand(
      "vasudev config patch --file <path> --dry-run",
    )}.`,
...
export function formatGatewayCommandFailure(params: {
  ...
  const message = raw
    .replace(/\s*Run [`"]?vasudev doctor[`"]? for diagnostics\.?/gi, "")
    .replace(/\s+Gateway target:\s+.*$/isu, "")
    .replace(/\s+/g, " ")
    .trim()
    .replace(/[.。]+$/u, "");
  const inspectCommand = params.inspectCommand ?? "vasudev gateway status --deep";
...
export function formatMissingPluginMessage(params: {
  ...
  const listCommand = params.listCommand ?? "vasudev plugins list";
  const searchHint = params.includeSearch
    ? `, or ${formatInlineCliCommand("vasudev plugins search " + params.id)} to look for installable plugins`
    : "";
