...
/** The Vasudev orb as a text glyph. It stands in for the mark the same way the
 * emoji did, and the decorative-emoji gate still hides it on terminals that
 * cannot render it. */
const BANNER_GLYPH = "◉";
...
export function formatCliBannerLine(version: string, options: BannerOptions = {}): string {
  ...
  const title = decorativePrefix(BANNER_GLYPH, PRODUCT_NAME, emojiOptions);
  const prefix = decorativeEmoji(BANNER_GLYPH, emojiOptions);
