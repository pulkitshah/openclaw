...
describe("cli program (nodes basics)", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        ...
        "vasudev nodes approve request-approval",
      ],
    },
    {
      ...
      expectedOutput: [
        ...
        "vasudev nodes approve request-reapproval",
      ],
    },
  ...
  it("runs nodes describe and calls node.describe", async () => {
    ...
    expect(out).toContain("vasudev nodes approve request-approval");
    expect(out).not.toContain("\u001b");
  ...
  it("keeps explicit gateway options in node reapproval guidance without leaking auth", async () => {
    ...
    expect(output).toContain(
      "vasudev --profile work nodes approve request-reapproval --timeout 3000",
    );
  ...
  it("does not recommend approval from a stale pending request id alone", async () => {
    ...
    expect(output).not.toContain("vasudev nodes approve stale-request");
