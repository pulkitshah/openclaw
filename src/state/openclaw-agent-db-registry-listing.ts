...
function readRegisteredAgentDatabases(
  ...
  const read = ({ db: database }: { db: import("node:sqlite").DatabaseSync }) => {
    ...
    if (!artifactPreserving && schemaMigrations.length > 0) {
      throw new Error(
        `Vasudev state database ${pathname} has a legacy agent database registry schema; run vasudev doctor --fix to migrate it.`,
      );
    }
    ...
    if (registryTable.type !== "table") {
      throw new Error(`Vasudev state database ${pathname} has an invalid agent registry.`);
    }
  ...
  if (entries === undefined) {
    if (hasUnavailableMissingSqlitePath(pathname)) {
      throw new Error(`Vasudev state database ${pathname} is unavailable.`);
    }
    return [];
  }
...
/** List agent databases recorded in the shared Vasudev state registry. */
