...
export function createOpenClawDatabaseVerificationError(
  ...
  const error = new Error(
    `Vasudev ${kind} database ${pathname} is quarantined after integrity verification failed: ${storedError ?? "unknown integrity error"}. Restore the database from a backup or repair it, then run vasudev doctor --fix to clear the quarantine. See ${OPENCLAW_DATABASE_SCHEMA_DOCS_URL}.`,
  );
...
function configureQuarantineWriter(database: DatabaseSync, storePath: string): void {
  ...
  if (userVersion > OPENCLAW_QUARANTINE_SCHEMA_VERSION) {
    throw new Error(
      `Vasudev quarantine store ${storePath} uses newer schema version ${userVersion}.`,
    );
  }
  if (userVersion === OPENCLAW_QUARANTINE_SCHEMA_VERSION) {
    return;
...
function readQuarantineSchemaVersion(database: DatabaseSync, storePath: string): number {
  ...
  if (typeof userVersion !== "number" || !Number.isInteger(userVersion)) {
    throw new Error(`Vasudev quarantine store ${storePath} has an invalid schema version.`);
  }
  return userVersion;
}
...
export function readOpenClawDatabaseQuarantine(
  ...
  try {
    ...
    if (
      ...
      throw new Error(`Vasudev quarantine store ${storePath} contains an invalid row.`);
    }
