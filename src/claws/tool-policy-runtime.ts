...
const uninitializedStateError = new Error(
  "Vasudev state database has not initialized Claw consent provenance.",
);
    ...
    super(
      options.unboundedFullProfile
        ? `Claw-managed agent ${JSON.stringify(agentId)} uses the legacy unbounded full tool profile. ` +
            "Add an explicit tools.allow list to its package Vasudev profile, then " +
            `run \`vasudev claws update ${agentId}\` and approve the refreshed tool authority.`
        : `Claw-managed agent ${JSON.stringify(agentId)} uses a legacy dynamic tool policy. ` +
            `Run \`vasudev claws update ${agentId}\` and approve the refreshed tool authority before running it.`,
    );
  ...
  constructor(agentId: string, cause: unknown) {
    super(
      `Cannot verify the installed tool authority for Claw-managed agent ${JSON.stringify(agentId)}. ` +
        "Repair the Vasudev state database before running it.",
      { cause },
    );
