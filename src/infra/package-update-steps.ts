  ...
  if (siblingPackages.length > 0) {
    return {
      ...
      failedStep: {
        ...
        exitCode: 1,
        stdoutTail: null,
        stderrTail: `Vasudev shares a pnpm ${owner.layoutVersion} global install group with ${siblingPackages.join(", ")}. Automatic update stopped before mutation; update the group manually to preserve its sibling packages.`,
      },
    };
  }
  ...
  if (!invokingPackageRoot || activePackageRoots.length !== 1 || ownerMatchCount !== 1) {
    return {
      ...
      failedStep: {
        ...
        exitCode: 1,
        stdoutTail: null,
        stderrTail: `Expected exactly one active pnpm ${owner.layoutVersion} Vasudev install owned by the invoking project; found ${activePackageRoots.length} active installs and ${ownerMatchCount} owner matches. Automatic update stopped before mutation.`,
      },
    };
  }
  ...
  if (
    ...
    return {
      ...
      failedStep: {
        ...
        stderrTail: `The active pnpm command owns ${reportedGlobalRoot || "an unknown global root"}, not the invoking Vasudev install at ${expectedGlobalRoot ?? "an unknown root"}. Automatic update stopped before mutation.`,
      },
    };
  }
...
>(
  ...
  const repairGuidance = "Run vasudev doctor --fix to finish deferred repairs.";
