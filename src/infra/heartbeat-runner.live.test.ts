...
describeLive("session event wake through a live Gateway", () => {
  it("continues a completed foreground session for exec results and keeps monitor polls on main", async () => {
    ...
    try {
      ...
      expect(completionUsers).toContain("[Vasudev exec completion]");
      expect(completionUsers).not.toContain("[Vasudev heartbeat poll]");
      ...
      await vi.waitFor(
        async () => {
          ...
          expect(messagesWithRole(mainMessages, "user")).toContain("[Vasudev heartbeat poll]");
