...
function assertStartupMigrationLeaseOwnedInTransaction(params: {
  ...
  if (!activeLease) {
    throw new Error(
      "Vasudev startup migration lease was lost before startup migrations completed; retry so migrations can run under a fresh lease.",
    );
  }
}
  ...
  writeStartupMigrationCheckpointDatabase(env, (db) => {
    ...
    if (existing && isStartupMigrationLeaseOwnerDefinitelyGone(existingOwner)) {
      ...
      throw new StartupMigrationLeaseConflictError(
        `Vasudev startup migrations are already running for this state directory; retry after the other Vasudev process finishes or after ${new Date(existing.expiresAt ?? expiresAt).toISOString()}.${ownerHint}`,
