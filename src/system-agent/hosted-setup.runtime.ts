...
export function requireLocalGateway(config: OpenClawConfig): void {
  ...
  throw new Error(
    "Hosted Gateway setup manages only a local Gateway. Use `vasudev onboard` for fresh setup or `vasudev configure` for the mode question, then retry after selecting local mode.",
  );
}
  ...
  return await runOutsidePluginRuntimeGenerationScope(() =>
    withPluginCache(cache, async (): Promise<HostedSetupCompletion> => {
      ...
      if (!snapshot.exists || !snapshot.valid || !snapshot.hash) {
        throw new Error(
          `${params.label} requires a valid saved config snapshot. On the machine running Vasudev, run \`vasudev doctor --fix\` and resolve any remaining validation errors; then retry.`,
        );
      }
...
export async function runHostedMemoryImport(
  ...
  if (!snapshot.exists || !snapshot.valid || !snapshot.hash) {
    throw new Error(
      "Memory import requires a valid saved config. Run `vasudev doctor --fix`, then retry.",
    );
  }
...
export async function renderMemoryImport(
  ...
  if (outcome.status === "workspace-missing") {
    return [
      ...
      "Finish onboarding first with `vasudev onboard`, then retry.",
    ].join("\n");
  }
