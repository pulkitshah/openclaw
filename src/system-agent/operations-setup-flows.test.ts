...
describe("system agent setup-flow operations", () => {
  ...
  it("prints one-shot setup pointers", async () => {
    ...
    expect(output).toContain("vasudev onboard`");
    expect(output).toContain("vasudev onboard --classic");
    expect(output).toContain("vasudev channels add --channel slack");
    expect(output).toContain("vasudev configure --section web");
    expect(output).toContain("vasudev configure --section gateway");
    expect(output).toContain("on the machine running Vasudev");
  });
  ...
  it("prints one-shot pointers for hosted skills, search, and Gateway setup", async () => {
    ...
    expect(lines.join("\n")).toContain("vasudev configure --section skills");
    expect(lines.join("\n")).toContain("vasudev configure --section web");
    expect(lines.join("\n")).toContain("vasudev configure --section gateway");
  });
  ...
  it("points one-shot memory import at interactive owners", async () => {
    ...
    expect(lines.join("\n")).toContain("vasudev onboard");
  });

  it("routes one-shot model setup through the verified Vasudev flow", async () => {
    const { runtime, lines } = createSystemAgentTestRuntime();

    const result = await executeSystemAgentOperation({ kind: "model-setup" }, runtime);

    expect(result.applied).toBe(false);
    expect(lines.join("\n")).toContain("Run `vasudev onboard` on the machine running Vasudev");
    expect(lines.join("\n")).toContain("Stop the Vasudev host");
    expect(lines.join("\n")).toContain("restart the host");
    expect(lines.join("\n")).not.toContain("vasudev configure --section model");
  });
