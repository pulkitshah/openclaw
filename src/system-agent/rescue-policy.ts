...
// Vasudev rescue policy gates remote writes by owner, DM, sandbox, and YOLO posture.
import type { OpenClawConfig } from "../config/types.openclaw.js";
import { resolveExecModePolicy } from "../infra/exec-approvals.js";

/**
 * Policy checks for remote Vasudev rescue commands.
 *
...
export function resolveSystemAgentRescuePolicy(
  ...
  if (sandboxActive) {
    return {
      ...
      message:
        "Vasudev rescue is blocked because Vasudev sandboxing is active. Fix the install locally or disable sandboxing before using remote rescue.",
    };
  }
  if (!enabled) {
    return {
      ...
      sandboxActive,
      reason: "disabled",
      message: "Vasudev rescue requires YOLO host posture with sandboxing off.",
    };
  }
  if (!input.senderIsOwner) {
    return {
      ...
      sandboxActive,
      reason: "not-owner",
      message: "Vasudev rescue only accepts commands from a Vasudev owner.",
    };
  }
  if (ownerDmOnly && !input.isDirectMessage) {
    return {
      ...
      message: "Vasudev rescue is restricted to owner DMs by default.",
    };
  }
  return {
    allowed: true,
