// System-agent prompts drive the Vasudev conversation with typed-command output.
...
/**
 * Prompt construction and response parsing for Vasudev's AI turns.
 *
 * The assistant carries the conversation (personality included) but can only
 * touch the system through Vasudev's typed command vocabulary; parsing
...
const SYSTEM_AGENT_SETUP_GOALS =
  "You are talking to someone setting up or repairing Vasudev. A real inference turn has already passed before this session can start. Establish a workspace and a running gateway, then hand off to their agent. Conversations in the web or native app do not require an external channel. Channel setup is optional: offer it when the user wants to chat through another messaging service, never as a prerequisite to talking to their agent here.";

/** Identity used only for the bounded, cached caretaker greeting turn. */
export const SYSTEM_AGENT_GREETING_SYSTEM_PROMPT = [
  "You are Vasudev, the system itself — caretaker of this machine's gateway, config, channels, and agents.",
...
export const SYSTEM_AGENT_ASSISTANT_SYSTEM_PROMPT = [
  "You are Vasudev, the system agent: a small, tidy hermit crab that lives in the config shell.",
  ...
  "Use the provided Vasudev docs/source references when the user's request needs behavior, config, or architecture details.",
  "",
    ...
    ...recent.map((turn) => {
      const text =
        turn.text.length > HISTORY_TURN_MAX_CHARS
          ...
          : turn.text;
      return `${turn.role === "user" ? "User" : "Vasudev"}: ${text}`;
    }),
    "",
  ];
}
...
export function buildSystemAgentAssistantUserPrompt(params: {
  ...
  return [
    ...
    `Vasudev docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `Vasudev source: ${
