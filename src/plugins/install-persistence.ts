...
function logShadowedNpmInstallWarning(params: {
  ...
  params.warn(
    [
      ...
      "Run `vasudev plugins doctor` for repair options.",
    ].join("\n"),
    `Installed plugin "${params.pluginId}" is shadowed by a configured plugin source. Run \`vasudev plugins doctor\`.`,
  );
}
  ...
  try {
    ...
    return await withPluginCache(createPluginCache(), async () => {
      ...
      if (manifests.length === 0) {
        throw new Error(
          `Plugin package "${params.pluginId}" has no authoritative runtime child list. Refresh the plugin registry, then reinstall the package or run vasudev doctor before retrying.`,
        );
      }
      ...
      if (replacedInstallRemoval) {
        const cleanup = async (
          ...
          reportWarning = (message: string) =>
            warn(
              message,
              "A previous plugin installation could not be fully cleaned up. Run `vasudev plugins doctor`.",
            ),
        ) => {
      ...
      const configWarning =
        params.enable !== false && configurationRequiredPluginIds.length > 0
          ? configurationRequiredPluginIds.length === 1
            ? `Installed plugin "${configurationRequiredPluginIds[0]}" without enabling it because it requires configuration first. Configure it, then run \`vasudev plugins enable ${configurationRequiredPluginIds[0]}\`.`
            : `Installed plugin entries ${configurationRequiredPluginIds.join(", ")} without enabling them because they require configuration first. Configure each entry, then run \`vasudev plugins enable <plugin-id>\`.`
          : undefined;
      const warningMessage = [params.warningMessage, configWarning].filter(Boolean).join("\n");
      if (warningMessage) {
        warn(
          warningMessage,
          configWarning ?? "Plugin installation reported a warning. Run `vasudev plugins doctor`.",
        );
      }
      runtime.log(
