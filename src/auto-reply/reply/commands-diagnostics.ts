...
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "vasudev gateway diagnostics export --json";
...
function isCodexDiagnosticsUnavailableText(text: string | undefined): boolean {
  return (
    text?.startsWith("No Codex thread is attached to this Vasudev session yet.") === true ||
    text?.startsWith(
      "Cannot send Codex diagnostics because this command did not include a Vasudev session file.",
    ) === true
  );
}
