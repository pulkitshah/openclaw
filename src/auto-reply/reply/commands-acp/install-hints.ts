...
export function resolveAcpInstallCommandHint(cfg: OpenClawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `vasudev plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^vasudev plugins install /u, "");
    ...
    return "vasudev plugins install acpx";
  }
