    ...
    JSON.stringify({
      ...(richManifest
        ? {
            ...
            schemaVersion: 1,
            description:
              "Extension manifest. Declares plugin packages that Vasudev can discover during onboarding and install on demand via `vasudev plugins install`.",
          }
        : {}),
      entries: [entry],
    }),
...
const [richNpmCatalogFixture, clawhubCatalogFixture, yuanbaoCatalogFixture] = [
  {
    ...
    entry: {
      ...
      description:
        "Vasudev WeCom (企业微信) channel plugin — community maintained, published on npm.",
      source: "external",
      kind: "channel",
      openclaw: {
        channel: {
  ...
  },
  {
    ...
    entry: {
      ...
      description:
        "Vasudev Yuanbao (元宝) channel plugin — community maintained, published on npm.",
      source: "external",
      kind: "channel",
      openclaw: {
        channel: {
