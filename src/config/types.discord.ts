...
export type DiscordVoiceRealtimeConfig = {
  ...
  /** Whether bidi should force the Vasudev agent brain for every substantive turn. */
  ...
  /** Wake names that allow OpenAI agent-proxy realtime Discord voice to respond when the gate is active. Defaults to the routed agent name plus Vasudev, or the agent id plus Vasudev. */
  wakeNames?: string[];
  ...
  /** Debounce window before buffered transcripts are sent to the Vasudev agent. */
  debounceMs?: number;
...
export type DiscordVoiceAgentSessionConfig = {
  /** Which Vasudev conversation should receive voice turns. Default: "voice". */
  mode?: "voice" | "target";
...
export type DiscordVoiceConfig = {
  ...
  /** Route voice turns through an existing Vasudev Discord conversation. */
