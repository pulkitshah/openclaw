...
describe("buildServiceEnvironment", () => {
  ...
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENCLAW_WINDOWS_TASK_NAME).toBe("Vasudev Gateway");
  ...
  it("uses profile-specific unit and label", () => {
    ...
    expect(env.OPENCLAW_WINDOWS_TASK_NAME).toBe("Vasudev Gateway (work)");
    if (process.platform === "darwin") {
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.OPENCLAW_WINDOWS_TASK_NAME).toBe("Vasudev Node");
    expect(env.OPENCLAW_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
