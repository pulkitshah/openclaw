...
describe("loadWebMedia", () => {
  ...
  it("allows trusted generated host-read HTML reports under Vasudev temp root", async () => {
  ...
  it("rejects host-read HTML files outside the trusted Vasudev temp root", async () => {
  ...
  it("rejects trusted host-read HTML symlinks that resolve outside Vasudev temp root", async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), "web-media-host-html-"));
  ...
  it("rejects trusted host-read HTML hardlinks to files outside Vasudev temp root", async () => {
