...
export function createPolicyDataAuthChecks(deps: PolicyDoctorCheckDeps): readonly HealthCheck[] {
  return createPolicyScopedChecks(deps, [
    ...
    [
      ...
      "Vasudev config SecretRefs use configured secret providers when policy requires managed providers.",
    ],
    [
      CHECK_IDS.policySecretsDeniedProviderSource,
      "Vasudev config secret providers and SecretRefs do not use sources denied by policy.",
    ],
    [
    ...
    [
      ...
      "Vasudev config auth profiles declare required provider and mode metadata.",
    ],
    [
      CHECK_IDS.policyAuthProfileUnapprovedMode,
      "Vasudev config auth profile modes stay within the policy allowlist.",
    ],
  ]);
}
