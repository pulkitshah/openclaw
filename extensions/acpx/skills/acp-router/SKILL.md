---
name: acp-router
description: Route plain-language requests for Claude Code, Cursor, Copilot, Vasudev ACP, OpenCode, Gemini CLI, Qwen, Kiro, Kimi, iFlow, Factory Droid, Kilocode, or explicit ACP harness work into either Vasudev ACP runtime sessions or direct acpx-driven sessions ("telephone game" flow). For coding-agent thread requests, read this skill first, then use only `sessions_spawn` for thread creation. Codex chat binding defaults to the native Codex app-server plugin unless ACP is explicit or background spawn needs ACP.
user-invocable: false
---

# ACP Harness Router

When user intent is "run this in Claude Code/Cursor/Copilot/Vasudev/OpenCode/Gemini/Qwen/Kiro/Kimi/iFlow/Droid/Kilocode (ACP harness)", do not use subagent runtime or PTY scraping. Route through ACP-aware flows.
...
Trigger this skill when the user asks Vasudev to:

- run something in Claude Code / Cursor / Copilot / Vasudev / OpenCode / Gemini / Qwen / Kiro / Kimi / iFlow / Droid / Kilocode
...
- After reading, follow `Vasudev ACP runtime path` below; do not use `message(action="thread-create")` for ACP harness thread spawn.

## Mode selection

Choose one of these paths:

1. Vasudev ACP runtime path (default): use `sessions_spawn` / ACP runtime tools.
...
- the task is "just relay prompts to harness" and no Vasudev ACP lifecycle features are needed
...
- `codex -> bundled @agentclientprotocol/codex-acp@1.10.0 through Vasudev's isolated CODEX_HOME wrapper`
