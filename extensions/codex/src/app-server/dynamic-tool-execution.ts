...
function formatDynamicToolTimeoutDetails(params: {
  ...
  if (tool !== "process" || !isJsonObject(params.call.arguments)) {
    return {
      responseMessage: `Vasudev dynamic tool call timed out after ${params.timeoutMs}ms while running tool ${tool}.`,
  ...
  return {
    responseMessage: `Vasudev dynamic tool call timed out after ${params.timeoutMs}ms${responseTarget}. This is a tool RPC timeout, not a session idle timeout.`,
...
export async function handleDynamicToolCallWithTimeout(params: {
  ...
  if (params.signal.aborted) {
    const message = "Vasudev dynamic tool call aborted before execution.";
  ...
  const abortFromRun = () => {
    const message = "Vasudev dynamic tool call aborted.";
    const terminalReason = resolveCodexToolAbortTerminalReason(params.signal);
    params.onFallbackSelected?.();
  ...
  try {
    ...
    const message = formatToolExecutionErrorMessage(error, "Vasudev dynamic tool call failed.");
    notifyFailedToolResult(message, terminalReason);
    ...
    if (!timedOut && !controller.signal.aborted) {
      controller.abort(new Error("Vasudev dynamic tool call finished."));
    }
  }
}
...
function readDynamicToolResponseText(response: CodexDynamicToolCallResponse): string {
  const text = response.contentItems
    ...
    .join("\n")
    .trim();
  return text || "Vasudev dynamic tool call failed.";
}

/** Strips Vasudev-only metadata before sending a dynamic tool response to Codex. */
...
export function resolveDynamicToolCallTimeoutMs(params: {
  ...
  if (
    ...
    try {
      ...
      // Vasudev delegation uses that default for its ten-minute approval plus
