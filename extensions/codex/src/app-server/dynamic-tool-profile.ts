...
/** Tool names owned by Codex app-server and normally excluded from Vasudev dynamic tools. */
...
/** Normalizes Vasudev/Codex tool names before filtering and allowlist checks. */
export function normalizeCodexDynamicToolName(name: string): string {
  const normalized = name.trim().toLowerCase();
  return DYNAMIC_TOOL_NAME_ALIASES[normalized] ?? normalized;
}

/** True only for the host-scoped Vasudev run's exact tool contract. */
...
/** Filters Vasudev tools that Codex owns natively or config explicitly excludes. */
...
/** Keeps Vasudev coding tools that replace a disabled Codex native surface. */
