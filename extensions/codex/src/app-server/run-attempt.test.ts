...
describe("runCodexAppServerAttempt", () => {
  ...
  it("starts active Vasudev sandbox threads with Codex native execution disabled", async () => {
  ...
  it("routes native Codex execution through a Vasudev sandbox exec-server when opted in", async () => {
    const appServer = {
      ...createThreadLifecycleAppServerOptions(),
      sandbox: "danger-full-access" as const,
    };
  ...
  it("starts Codex threads without duplicate Vasudev workspace tools by default", async () => {
    const { sessionFile, workspaceDir } = createRunPaths();
  ...
  it("includes Codex app-server scoped plugin command guidance in developer instructions", () => {
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "Vasudev main command guidance.",
    ...
    expect(instructions).not.toContain("Vasudev main command guidance.");
  });
  it("passes Vasudev skills as turn collaboration developer instructions", async () => {
    ...
    const collaborationInstructions =
      turnStartParams.collaborationMode?.settings?.developer_instructions ?? "";
    expect(collaborationInstructions).toContain("## Vasudev Skills");
    expect(collaborationInstructions).toContain("<available_skills>");
    const inputText = turnStartParams.input?.[0]?.text ?? "";
    expect(inputText).not.toContain("## Vasudev Skills");
    ...
    expect(compiledContext?.data?.systemPrompt).toContain("## Vasudev Skills");
  ...
  it("emits TUI-compatible tool events for Codex dynamic tool calls", async () => {
    ...
    expect(firstResponse).toMatchObject({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown Vasudev tool: python" }],
    });
    ...
    expect(onRunAgentEvent).toHaveBeenCalledWith({
      ...
      data: {
        ...
        isError: true,
        result: {
          content: [{ type: "text", text: "Unknown Vasudev tool: python" }],
        },
      },
    });
  ...
  it("keeps leading delivery hints out of the Codex current user request", async () => {
    for (const [index, deliveryHint] of MESSAGE_TOOL_DELIVERY_HINTS.entries()) {
      ...
      const inputText = turnStartParams.input?.[0]?.text ?? "";
      expect(inputText).toContain("Vasudev delivery metadata:");
  ...
  it("keeps Vasudev control-path tools direct when code-mode-only is enabled", () => {
    const tools = [
      createRuntimeDynamicTool("message"),
  ...
  it("keeps the heartbeat schema deferred and stable across normal and heartbeat turns", async () => {
    ...
    expect(normalInstructions).not.toContain(
      "Deferred searchable Vasudev dynamic tools available: heartbeat_respond",
    );
  ...
  it("keeps message in the registered schema when disabled for an internal turn", async () => {
    ...
    await expect(
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Vasudev tool is not available for this turn: message",
        },
      ],
    });
  });
  ...
  it("does not project mirrored messages on consecutive resumes", async () => {
    ...
    const firstInputText =
      (firstTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(firstInputText).toContain("Vasudev assembled context for this turn:");
  ...
  it("routes AGENTS.md natively and MEMORY.md through tools", async () => {
    ...
    expect(collaborationInstructions).toContain("Vasudev Agent Soul");
    ...
    expect(collaborationInstructions).toContain("Vasudev Workspace Memory");
    ...
    expect(inputText).not.toContain("Vasudev runtime context for this turn:");
    ...
    expect(inputText).not.toContain("Vasudev Workspace Memory");
  ...
  it("adds memory recall guidance when dated memory notes exist without root MEMORY.md", async () => {
    ...
    expect(collaborationInstructions).toContain("memory_search");
    expect(collaborationInstructions).toContain("memory_get");
    expect(collaborationInstructions).not.toContain("Vasudev Workspace Memory");
  ...
  it("inherits external-cwd agent instructions through Codex thread creation", async () => {
    ...
    expect(threadInstructions).toContain("Vasudev Agent Workspace Instructions");
  ...
  it.each([
    ...
    const collaborationInstructions =
      turnStartParams.collaborationMode?.settings?.developer_instructions ?? "";
    expect(collaborationInstructions).not.toContain("This is a Vasudev heartbeat turn");
  ...
  it("keeps lightweight cron Codex turns out of Vasudev bootstrap context", async () => {
    const { sessionFile, workspaceDir } = createRunPaths();
    const exactCommand =
  ...
  it("keeps lightweight cron delivery hints byte-for-byte without Vasudev prompt context", async () => {
  ...
  it("keeps effective default Codex yolo when Vasudev tool policy exists", async () => {
    initializeGlobalHookRunner(
  ...
  it("keeps explicit Codex yolo mode unpromoted when Vasudev tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const { sessionFile, workspaceDir } = createRunPaths();
  ...
  it("keeps normalized full exec mode unpromoted when Vasudev tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const { sessionFile, workspaceDir } = createRunPaths();
