 ...
 * Vasudev-owned tools, Codex native-tool fallback rules, sandbox shell shims,
...
/** Factory seam for constructing Vasudev runtime tools without eagerly loading agent-harness. */
...
/** Keeps the Vasudev Gateway execution path available beside Codex native shell. */
...
export function shouldEnableCodexAppServerNativeToolSurface(
  ...
  // capability, so narrow Vasudev allowlists must fail closed rather than
...
function canCodexAppServerNativeToolSurfaceHonorSandbox(
  ...
  // active Vasudev sandboxing must disable the native surface and route shell
...
/** Requires a Codex sandbox environment only when native tools must run inside Vasudev sandboxing. */
...
/** Converts Vasudev sandbox networking into Codex's external-sandbox policy shape. */
...
function addSandboxShellDynamicToolsIfAvailable(
  ...
  const sandboxExecTool: OpenClawDynamicTool = {
    ...
    name: "sandbox_exec",
    description:
      "Run a shell command through Vasudev's configured sandbox backend for this session. Use when Vasudev sandboxing is active or when a command must execute in the sandbox backend, such as an SSH-backed sandbox or Docker container-path bind layout. Use Codex's native shell only when no Vasudev sandbox is active and native Code Mode is available.",
  ...
  const sandboxProcessTool: OpenClawDynamicTool = {
    ...
    description:
      "Manage background shell sessions through Vasudev's configured sandbox backend for this session: list, poll, log, write, send-keys, submit, paste, kill, clear, or remove. Use only for sandbox follow-up; use Codex's native shell session handling only when no Vasudev sandbox is active and native Code Mode is available.",
  };
...
function shouldKeepOpenClawShellDynamicTools(
  ...
  return (
    ...
    // advertise a shell. Preserve Vasudev's policy-filtered direct shell.
    input.nativeToolSurfaceEnabled === false &&
    input.sandbox?.enabled !== true &&
