    ...
    expect(JSON.parse(harness.writes[0] ?? "{}")).toEqual({
      ...
      result: {
        ...
        contentItems: [
          {
            type: "inputText",
            text: `Vasudev dynamic tool call timed out after ${CODEX_DYNAMIC_TOOL_SERVER_REQUEST_TIMEOUT_MS}ms before sending a response to Codex.`,
          },
        ],
      },
    });
  ...
  it("returns an explicit bounded decline for unhandled MCP elicitations", async () => {
    ...
    expect(JSON.parse(harness.writes[0] ?? "{}")).toEqual({
      ...
      result: {
        action: "decline",
        content: null,
        _meta: { message: "Vasudev has no interactive handler for this elicitation." },
      },
    });
  });
});
