...
if (gatewayAttempts === 1 && process.env.QA_STARTUP_RETRY) {
  process.stderr.write(process.env.QA_STARTUP_RETRY === "migration"
    ? "Vasudev plugin migration inputs changed during startup convergence; refusing readiness."
...
describe("runQaGatewayCliCommand", () => {
  ...
  it("reports CLI stderr when a fixture command fails", async () => {
    ...
    ).rejects.toThrow("Vasudev CLI exited 7: fixture failure");
  });
  ...
  it("retains bounded redacted stdout failures alongside stderr panels", async () => {
    ...
    try {
      ...
      expect(error.message).toContain("Vasudev CLI exited 7: Doctor panel:");
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("cleans up temp QA gateway roots when repo CLI discovery fails before startup", async () => {
    ...
    ).rejects.toThrow("Vasudev CLI entry not found");
    await expect(owner.stop()).resolves.toMatchObject({ errors: [] });
  ...
  it.each(["parent", "runtime patch"])(
    ...
    (source) => {
      const supervisorEnv = {
        ...
        OPENCLAW_WINDOWS_TASK_NAME: "Vasudev Gateway",
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing Vasudev home", async () => {
  ...
  it("forwards host HOME for live Claude CLI runs while keeping Vasudev home sandboxed", async () => {
    const hostHome = await tempDirs.makeTempDir("qa-host-home-");
  ...
  it("can forward host HOME for browser-backed QA runs while keeping Vasudev home sandboxed", async () => {
    const hostHome = await tempDirs.makeTempDir("qa-host-home-");
  ...
  it.each(["openai", "anthropic", "help", "repair"] as const)(
    ...
    async (phase) => {
      ...
      const detail = provider
        ? "Vasudev CLI exited 9: Authorization: Bearer <redacted>"
        : "Vasudev CLI exited 8: plugin fixture rejected: Authorization: Bearer <redacted>";
  ...
  it.each([
    ...
    [
      "Vasudev plugin migration inputs changed during startup convergence; refusing to report the gateway ready. Restart Vasudev so state migrations run against the final config and plugin inventory.",
  ...
  it.each([
    ...
    "Restart Vasudev so state migrations can continue.",
