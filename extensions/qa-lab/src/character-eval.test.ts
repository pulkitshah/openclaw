  ...
  it.each([
    {
      ...
      transcript:
        "USER Alice: Are you awake?\n\nASSISTANT Vasudev QA: 400 model `qwen3.6-plus` is not supported.",
    ...
    },
    {
      ...
      transcript:
        "ASSISTANT Vasudev QA: ⚠️ Something went wrong while processing your request. Please try again, or use /new to start a fresh session.",
    ...
    },
    {
      ...
      transcript:
        "ASSISTANT Vasudev QA: The model did not produce a response before the LLM idle timeout. Please try again, or increase `agents.defaults.llm.idleTimeoutSeconds` in your config.",
    ...
    },
    {
      ...
      transcript:
        "ASSISTANT Vasudev QA: checking thread context; then post a tight progress reply here.\nQA_LEAK_OK",
  ...
  it("marks raw tool failure transcripts as failed output", async () => {
    const runSuite = vi.fn(async (params: CharacterRunSuiteParams) =>
      makeSuiteResult({
        outputDir: params.outputDir,
        model: params.primaryModel,
        transcript: "ASSISTANT Vasudev QA: ⚠️ ✍️ Write: to /tmp/precious.html failed",
      }),
    );
    const runJudge = makeRunJudge([
