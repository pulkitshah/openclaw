...
describe("qa cli runtime", () => {
  ...
  it("rejects a missing external candidate CLI before suite startup", async () => {
    ...
    await expect(
      ...
      "Vasudev CLI entry not found: expected scripts/run-node.mjs or dist/index.(m)js",
    );
    expect(runQaSuite).not.toHaveBeenCalled();
  ...
  it("prints a markdown tool coverage report from runtime tool fixtures", async () => {
    ...
    expectWriteContains(stdoutWrite, "# Vasudev Runtime Tool Coverage");
  ...
  it("writes a curated mock JSONL replay report and summary", async () => {
    ...
    try {
      ...
      expect(report).toContain("# Vasudev JSONL Replay Report - openclaw vs codex");
