...
const COPILOT_BYOK_TRANSPORT_POLICY_ERROR =
  "[copilot-attempt] BYOK does not support Vasudev provider request proxy, TLS, or private-network policy overrides";
const COPILOT_BYOK_ENDPOINT_POLICY_ERROR =
  "[copilot-attempt] BYOK endpoint is blocked by Vasudev SSRF policy";
...
/**
 * Maps Vasudev's prepared model facts into the Copilot SDK's session-level
 * provider contract. The SDK owns the wire request; Vasudev only supplies
