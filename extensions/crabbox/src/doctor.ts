        ...
        for (const image of listCrabboxWarmImages(ctx.env)) {
          ...
          if (image.retirement) {
            findings.push({
              ...
              fixHint:
                "Cleanup retries during the next warm-image capture or worker teardown. Inspect `vasudev crabbox warm-images --json` and resolve provider deletion errors if it remains pending.",
            });
          }
        }
        return findings;
