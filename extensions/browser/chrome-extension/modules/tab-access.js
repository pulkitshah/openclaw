...
export function createTabAccessPolicy({ chromeApi = chrome, isSelectedTab, getGroupColor }) {
  ...
  async function requireTab(tabId, epoch = capture(tabId), afterNavigation = false) {
    ...
    if (state.reason === "paused") {
      throw new Error(`tab ${tabId} is paused for Vasudev`);
    }
    if (state.reason === "not-selected") {
      throw new Error(`tab ${tabId} is not in the Vasudev tab group`);
    }
    if (state.reason === "incognito") {
      throw new Error(`tab ${tabId} is incognito and unavailable to Vasudev`);
    }
    throw new Error(`tab ${tabId} is restricted or unavailable to Vasudev`);
  }
