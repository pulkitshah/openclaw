  ...
  if (!printBrowserJsonResult(parent, result)) {
    ...
    if (result.blockedByDialog) {
      ...
      const ids = Array.isArray(pending)
        ...
        : [];
      defaultRuntime.log(
        `Action blocked by a modal dialog${ids.length ? ` (${ids.join(", ")})` : ""}. Use vasudev browser dialog --accept or --dismiss${ids.length ? " --dialog-id <id>" : ""} to continue.`,
      );
...
function createActionsInputTooLargeError(source: string, cause?: unknown): FsSafeError {
  return new FsSafeError(
    "too-large",
    `${source} exceeds ${ACTIONS_INPUT_MAX_BYTES} bytes. Split the batch plan into smaller files or run multiple vasudev browser batch commands.`,
    { cause },
  );
}
