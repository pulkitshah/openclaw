  ...
  for (const message of params.messages) {
    await store.register(`${scopeKey}:default:${params.chatId}:${message.messageId}`, {
      ...
      sourceMessage: {
        ...
        date: message.date,
        from: { id: message.unversioned ? 0 : 999, is_bot: true, first_name: "Vasudev" },
...
describe("createTelegramBot", () => {
  ...
  it("dedupes outbound prompt-context sends with ambient group history", async () => {
    ...
    await recordOutboundMessageForPromptContext({
      cfg,
      account: { accountId: "default", name: "Vasudev" },
      chatId: -42,
      message: {
    ...
    expect(payload.InboundHistory).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ body: "Bot just replied", sender: "Vasudev (you)" }),
      ]),
    );
    const [conversationContext] = requireArray(
      payload.ChannelStructuredContext,
    ...
    try {
      ...
      await handler(
        makeDirectTelegramMessageContext({
          ...
          senderId,
          context: {
            me: { id: 999, is_bot: true, first_name: "Vasudev", username: "openclaw_bot" },
          },
          message: {
            text: "still there?",
            reply_to_message: {
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "Vasudev" },
        },
      },
      me: { id: 999, username: "openclaw_bot" },
      getFile: getEmptyTelegramFile,
