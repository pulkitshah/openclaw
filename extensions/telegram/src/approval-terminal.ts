...
export function buildTelegramCanonicalApprovalTerminalText(params: {
  ...
  if (approval.presentation?.kind === "system-agent" && params.result.applied) {
    if (approval.status === "allowed") {
      return `✅ Vasudev change approved. Applying: ${truncateDetail(approval.presentation.description)}`;
    }
    if (approval.status === "cancelled") {
      return "⚠️ Vasudev change was cancelled because its run ended. No change was made. Retry.";
    }
    if (approval.status === "denied") {
      return "❌ Vasudev change denied. No change was made.";
    }
